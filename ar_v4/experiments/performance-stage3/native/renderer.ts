...
import type {GpuNativeFrame, GpuFrameMetrics} from './gpu-frame.ts';
import type {NativeImagePair} from '../../performance-stage2/native/shared-readback.ts';
...
const emptyStageTimings = (): NativeStageTimings => ({setupMs: 0, visibilitySubmitMs: 0, beautySubmitMs: 0,
  ...
  requestedCamera: false, sharedCameraReady: false, sharedCameraFailure: null, gpu: null});
...
export class TryOnRenderer {
  ...
  private gpuFrames: NativeGpuFrames | null = null;
  private currentGpuFrame: GpuNativeFrame | null = null;
  private nativePair: NativeImagePair | null = null;
  ...
  get nativeImagePair(): NativeImagePair | null {return this.disposed ? null : this.nativePair;}
  get gpuFrame(): GpuNativeFrame | null {return this.disposed ? null : this.currentGpuFrame;}
  /** Higher-level preparation can fail before native.present is reached. The
   * wrapper revokes its prior lease at the start of every preparation attempt. */
  invalidateGpuFrame(): void {
    this.currentGpuFrame = null; this.nativePair = null; this.gpuFrames?.invalidate();
  }
  get stageTimings(): NativeStageTimings {return structuredClone({...this.stages, gpu: this.gpuFrames?.metrics ?? this.stages.gpu});}
  ...
  present(frame: HTMLCanvasElement, detection: Detection, recordedSurface?: readonly number[],
    ...
    try {
      ...
      try {
        ...
        this.gpuFrames ??= new NativeGpuFrames(this.renderer);
        this.gpuFrames.begin(renderWidth, renderHeight);
        this.stages.readbackSetupMs = performance.now() - readbackStarted;
        // Resolve the original default buffer before another pass touches it.
        // Even no-face/hair-off pairs can publish this private GPU beauty.
        this.gpuFrames.capture('beauty');
        if (captureCamera && this.facePose.visible) {
          ...
          this.gpuFrames.capture('camera');
          this.stages.sharedCameraReady = true;
        }
        this.currentGpuFrame = this.gpuFrames.lease();
      } catch (error) {
        // Failed optional capture never licenses a stale texture or camera-only
        // fallback. Render the original beauty afresh for the checked CPU path.
        this.currentGpuFrame = null; this.gpuFrames?.invalidate(); this.nativePair = null;
        this.stages.sharedCameraReady = false;
        this.stages.sharedCameraFailure = error instanceof Error ? error.message : 'Native GPU capture failed';
        const fallbackStarted = performance.now();
        this.renderer.render(this.scene, this.camera);
        this.stages.fallbackBeautySubmitMs = performance.now() - fallbackStarted;
      } finally {
        this.cleanScene.background = null;
        this.stages.gpu = this.gpuFrames?.metrics ?? null;
      }
  ...
  dispose(): void {
    ...
    this.cleanScene.background = null;
    this.nativePair = null; this.currentGpuFrame = null; this.gpuFrames?.dispose(); this.gpuFrames = null;
