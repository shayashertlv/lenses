...
import {LiveHairRenderer as Stage2Renderer} from './renderer.ts';
...
export class ComparisonRenderer {
  ...
  private readonly renderedRevision: Record<Pipeline, number> = {current: -1, test: -1, test2: -1};
  private revision = 0;
  ...
  static async create(display: HTMLCanvasElement, signal: AbortSignal, eyewearId: EyewearId = DEFAULT_EYEWEAR_ID): Promise<ComparisonRenderer> {
    ...
    try {
      ...
      return new ComparisonRenderer({current, test, test2});
    } catch (error) {test2?.dispose();test?.dispose();current?.dispose();throw error;}
  }
  ...
  exportDiagnostic(): Record<string, unknown> | null {
    ...
    try {
      ...
      return {schema: 'ar-performance-stage2-comparison-v1', candidateAccepted: false, selectedPipeline: selected,
        inputPolicy: 'All three outputs use one exact held source, detection and hair mask. Only the selected pipeline runs live.',
        ...outputs};
    } finally {
