export const PIPELINES = ['current', 'test', 'test2'] as const;
export type Pipeline = typeof PIPELINES[number];
export const PIPELINE_LABELS: Record<Pipeline, string> = {current: 'Long-hair checkpoint', test: 'Test 1', test2: 'Current · Test 2'};
...
export class FrameProfiler {
  ...
  /** Only three small cached summaries; never scans or copies per-frame samples. */
  ...
  progress(at: number): string {
    const run=this.benchmark;
    if (!run) return 'Measure each algorithm for 30 seconds after warmup. The comparison takes about two minutes.';
  ...
  snapshot(): Record<string, unknown> {
    const run=this.benchmark;
    return {schema:'ar-stage2-performance-v1',timeOriginMs:performance.timeOrigin,
