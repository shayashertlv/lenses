...
const testUrl = new URL('../performance-candidate/renderer.ts', import.meta.url).href;
const stage2Url = new URL('./renderer.ts', import.meta.url).href;
const eyewearUrl = new URL('../../references/perfect-temples/src/render/eyewear.ts', import.meta.url).href;
const factorySymbol = Symbol.for('performance-stage2-comparison-ownership-factories');
const hooks = registerHooks({load(url, context, nextLoad) {
  ...
  if (url === currentUrl || url === testUrl || url === stage2Url) return {format: 'module', shortCircuit: true,
    source: `export class LiveHairRenderer { static create(...args) {
      return globalThis[Symbol.for('performance-stage2-comparison-ownership-factories')].${url === currentUrl ? 'current' : url === testUrl ? 'test' : 'test2'}(...args);
    } }`};
...
function fixture() {
  ...
  const current = new StubRenderer(display, 11), candidate = new StubRenderer(display, 22), stage2 = new StubRenderer(display, 33);
  const renderers = {current, test: candidate, test2: stage2};
  const renderer = Reflect.construct(ComparisonRenderer, [renderers]) as Comparison;
  ...
  return {renderer, renderers, current, candidate, stage2, source, canvas, display, detection, pair, model, mask};
}
...
test('a live switch during pending work commits only on next prepare and never runs the inactive renderer', t => {
  ...
  assert.equal(f.current.prepares, 1); assert.equal(f.candidate.prepares, 0); assert.equal(f.stage2.prepares, 0);
  ...
  f.renderer.selectPipeline('test2'); f.renderer.finish(f.mask);
  assert.equal(f.display.pixels[0], 22); assert.equal(f.renderer.pipeline, 'test');
  f.renderer.prepare(f.canvas, f.detection, f.pair, f.model);
  assert.equal(f.renderer.pipeline, 'test2'); assert.equal(f.current.prepares, 1); assert.equal(f.candidate.prepares, 1);
  assert.equal(f.stage2.prepares, 1); f.renderer.selectPipeline('current'); f.renderer.finish(f.mask);
  assert.equal(f.display.pixels[0], 33); assert.equal(f.renderer.pipeline, 'test2');
  f.renderer.present(f.canvas, f.detection, f.mask, f.pair, f.model);
  assert.equal(f.renderer.pipeline, 'current'); assert.equal(f.current.prepares, 2); assert.equal(f.candidate.prepares, 1); assert.equal(f.stage2.prepares, 1);
});

test('held switches reuse one exact owned image/detection/mask and full-mask upgrade invalidates all three output revisions', t => {
  const f = fixture(); t.after(() => f.renderer.dispose());
  ...
  f.renderer.selectPipeline('test2');
  assert.deepEqual(f.stage2.preparedInputs[0]!.pair, savedPair);
  assert.deepEqual(f.stage2.preparedInputs[0]!.detection, savedDetection);
  assert.equal(f.stage2.preparedInputs[0]!.source.pixels[0], 37);
  assert.equal(f.stage2.receivedMasks[0], f.mask);
  f.renderer.selectPipeline('current'); f.renderer.selectPipeline('test'); f.renderer.selectPipeline('test2');
  assert.equal(f.current.prepares, 1); assert.equal(f.candidate.prepares, 1); assert.equal(f.stage2.prepares, 1);
  ...
  f.renderer.selectPipeline('test'); f.renderer.selectPipeline('current');
  assert.equal(f.current.prepares, 2); assert.equal(f.candidate.prepares, 2); assert.equal(f.stage2.prepares, 2);
  assert.equal(f.current.receivedMasks[1], full); assert.equal(f.candidate.receivedMasks[1], full); assert.equal(f.stage2.receivedMasks[1], full);
  ...
  assert.equal(output.schema, 'ar-performance-stage2-comparison-v1');
  assert.deepEqual(output.current, output.test); assert.deepEqual(output.current, output.test2);
  for (const renderer of Object.values(f.renderers)) assert.equal(renderer.prepares, 2, 'export reuses every completed held revision');
  held.source.width = held.source.height = 0;
});

test('Test2 skips hair for live accepted preparation but full held upgrade and inactive export explicitly request it', t => {
  const f = fixture(); t.after(() => f.renderer.dispose());
  f.renderer.selectPipeline('test2'); f.renderer.selectVariant('accepted');
  f.renderer.present(f.canvas, f.detection, null, f.pair, f.model); f.renderer.setHeld();
  assert.deepEqual(f.stage2.hairPreparationRequests, [false]);
  const held = f.renderer.copyHeldInput()!;
  const full: HairMask = {...f.mask, outputMode: 'full', confidence: new Float32Array([.1, .9, .9, .1]), confidenceSHA256: '5'.repeat(64)};
  f.renderer.present(held.source, held.detection, full, held.pair, held.expectedModel);
  assert.equal(f.renderer.variant, 'accepted'); assert.deepEqual(f.stage2.hairPreparationRequests, [false, true]);
  const output = f.renderer.exportDiagnostic()!;
  assert.deepEqual(output.current, output.test); assert.deepEqual(output.current, output.test2);
  assert.deepEqual(f.current.hairPreparationRequests, [true]); assert.deepEqual(f.candidate.hairPreparationRequests, [true]);
  assert.equal(f.renderer.pipeline, 'test2'); assert.equal(f.display.pixels[0], 33);
  held.source.width = held.source.height = 0;

  // Also exercise Test2 as an inactive target first prepared from an upgraded mask.
  const g = fixture(); t.after(() => g.renderer.dispose());
  g.renderer.selectVariant('accepted'); g.renderer.present(g.canvas, g.detection, full, g.pair, g.model); g.renderer.setHeld();
  g.renderer.exportDiagnostic(); assert.deepEqual(g.stage2.hairPreparationRequests, [true]);
});

test('inactive held preparation, finish and export failures restore the selected display and permit retry', t => {
  for (const pipeline of ['test', 'test2'] as const) for (const failure of ['failPrepare', 'failFinish', 'failExport'] as const) {
    const f = fixture(); t.after(() => f.renderer.dispose());
    f.renderer.present(f.canvas, f.detection, f.mask, f.pair, f.model); f.renderer.setHeld();
    const pixels = f.display.pixels.slice(); f.renderers[pipeline][failure] = true;
    ...
    assert.ok(Object.values(f.renderers).flatMap(renderer => renderer.issuedSources).every(source => source.width === 0 && source.height === 0));
    f.renderers[pipeline][failure] = false;
    const output = f.renderer.exportDiagnostic()!;
    assert.deepEqual(output.current, output.test); assert.deepEqual(output.current, output.test2); assert.deepEqual(f.display.pixels, pixels);
  }
});

test('failure while exporting Current or Test1 restores a previously selected Test2 output', t => {
  for (const pipeline of ['current', 'test'] as const) {
  const f = fixture(); t.after(() => f.renderer.dispose());
  f.renderer.present(f.canvas, f.detection, f.mask, f.pair, f.model); f.renderer.setHeld(); f.renderer.selectPipeline('test2');
  const pixels = f.display.pixels.slice(); f.renderers[pipeline].failExport = true;
  assert.throws(() => f.renderer.exportDiagnostic(), /injected export/);
  assert.equal(f.renderer.pipeline, 'test2'); assert.equal(f.renderer.requestedPipeline, 'test2');
...
test('no-face input drops prior owned facial metadata and disposal cancels pending publication exactly once', t => {
  ...
  assert.equal(f.current.disposals, 1); assert.equal(f.candidate.disposals, 1); assert.equal(f.stage2.disposals, 1);
...
test('cancellation while the third factory is pending releases late initialization and all three renderers', async t => {
  ...
  const thirdStarted = new Promise<void>(resolve => {entered = resolve;});
  Object.defineProperty(globalThis, factorySymbol, {configurable: true, value: {
    current: async () => f.current,
    test: async () => f.candidate,
    test2: async () => {entered!(); await gate; return f.stage2;},
  }});
  ...
  await thirdStarted; abort.abort(); release!();
  await assert.rejects(pending, {name: 'AbortError'});
  assert.equal(f.current.disposals, 1); assert.equal(f.candidate.disposals, 1); assert.equal(f.stage2.disposals, 1);
  assert.equal(f.current.prepares, 0); assert.equal(f.candidate.prepares, 0); assert.equal(f.stage2.prepares, 0);
});

test('third factory failure releases both previously initialized renderers', async t => {
  const f = fixture(), previous = Object.getOwnPropertyDescriptor(globalThis, factorySymbol);
  Object.defineProperty(globalThis, factorySymbol, {configurable: true, value: {
    current: async () => f.current, test: async () => f.candidate,
    test2: async () => {throw new Error('injected third factory failure');},
  }});
  t.after(() => {f.renderer.dispose(); if (previous) Object.defineProperty(globalThis, factorySymbol, previous); else Reflect.deleteProperty(globalThis, factorySymbol);});
  await assert.rejects(ComparisonRenderer.create(f.display as unknown as HTMLCanvasElement, new AbortController().signal), /third factory failure/);
  assert.equal(f.current.disposals, 1); assert.equal(f.candidate.disposals, 1); assert.equal(f.stage2.disposals, 0);
});
