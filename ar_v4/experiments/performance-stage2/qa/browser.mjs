  ...
  return {
    ...
    async render(input) {
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        diagnosticChecks[name] = {pair: equal(diagnostic.pair, pair), detection: equal(diagnostic.detection, source.detection),
          ...
          category: await hash(Uint8Array.from(atob(diagnostic.mask.categoryBase64), c => c.charCodeAt(0))) === mask.categorySHA256,
          geometryRawPose: equal(geometry.rawMatrix, source.detection.matrix)};
        const stats = renderer.stats;
        check(stats?.hasMask && stats.maskOutputMode === 'category-only' && !stats.fallbackReason, `${name}: paired mask was rejected.`);
        if (name === 'candidate') {
        ...
        for (const field of ['backgroundReferenceCheck', 'protectedCheck', 'noseCheck', 'outsideEditableCheck', 'backgroundPreservationCheck'])
          check(stats[field]?.changedPixels === 0, `${name}: ${field} failed.`);
      ...
      return {id: input.id, eyewearModel: eyewear, hairModel: input.hairModel, pair, width: source.canvas.width, height: source.canvas.height,
        passed, checks, geometryChecks, diagnosticChecks, samples, pngs,
    ...
    async controls(input) {
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        const hairOffNoAtlas = name !== 'candidate' || hairOffNative?.sharedCameraReady === false
        ...
        const noFaceNoAtlas = name !== 'candidate' || noFaceNative?.sharedCameraReady === false
