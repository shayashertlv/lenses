...
const base = option('base', 'http://127.0.0.1:8072').replace(/\/$/, ''), phase = option('phase', 'all');
assert.equal(new URL(base).hostname, '127.0.0.1', 'QA attaches only to an explicit local server.');
const currentModule = option('current', '/experiments/performance-candidate/renderer.ts');
const candidateModule = option('candidate', '/experiments/performance-stage2/renderer.ts');
...
async function scan(directory) {
  for (const entry of await fs.readdir(directory, {withFileTypes: true})) {
    if (['output', 'node_modules', '.recovery', 'dist', '.git'].includes(entry.name)) continue;
    const filename = path.join(directory, entry.name);
    if (entry.isDirectory()) await scan(filename);
    else if (/\.(ts|mjs|html|css|json|glb|tflite|task|wasm|js)$/.test(entry.name)) runtime.set(filename, sha(await fs.readFile(filename)));
  }
}
...
if (args.includes('--preflight')) {
  ...
  console.log(JSON.stringify({preflight: true, frozenFilesVerified: frozen.size, runtimeFilesVerified: runtime.size, implementationLabels: {current: 'Test1', candidate: 'Test2'}, phases: plans.map(plan => ({id: plan.id, images: plan.sources.length, masks: plan.masks.length, cases: plan.cases.length}))}));
  process.exit(0);
}
const out = path.join(workspace, 'experiments/performance-stage2/qa/output', `matched-${new Date().toISOString().replaceAll(':', '-')}`);
await fs.mkdir(out, {recursive: true});
const report = {schema: diagnosticCase ? 'ar-performance-stage2-bounded-repeat-v1' : 'ar-performance-stage2-matched-v1', complete: false, passed: false, createdAt: new Date().toISOString(),
  base, currentModule, candidateModule, implementationLabels: {current: 'Test1', candidate: 'Test2'}, out, phases: [], runtimeHashes: Object.fromEntries(runtime),
  ...
  limits: ['No face or hair inference is rerun. Renderer wall times do not establish live FPS, face/hair contention, sustained mobile speed or physical-camera latency.',
    ...
    'The production live scheduler and three-way switching are checked separately.',
    'The prior Test1 warmed current-renderer protected-pixel variance is retained and is not resolved by passing this later Test1/Test2 still comparison.']};
...
try {
  for (const plan of plans) {
    ...
    measured.environment = await page.evaluate(async parameters => {
      const {createMatchedStudy} = await import('/experiments/performance-stage2/qa/browser.mjs');
    ...
    }, {currentModule, candidateModule, generated: plan.generated});
    ...
    for (const required of ['/experiments/performance-candidate/native/renderer.ts', '/experiments/performance-stage2/native/renderer.ts',
