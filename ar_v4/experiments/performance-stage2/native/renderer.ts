...
import type {NativeImagePair, SharedReadbackMetrics} from './shared-readback.ts';
    ...
    try {
      ...
      if (captureCamera && this.facePose.visible) {
        try {
          ...
          this.pairedReadback ??= new SharedNativeReadback(this.renderer);
          this.pairedReadback.begin(renderWidth, renderHeight);
          ...
          this.pairedReadback.capture(1);
          this.nativePair = this.pairedReadback.read();
          ...
          this.stages.sharedReadback = this.pairedReadback?.metrics ?? null;
        }
      }
