...
interface Comparison {current: Output; test: Output; selectedPipeline: string; candidateAccepted: boolean;}
...
async function collect(page: Page, pipeline: 'current' | 'test'): Promise<Record<string, unknown>[]> {
...
for (const eyewear of ['amber-horizon', 'tom-ford-clear']) for (const hair of ['hair-only', 'selfie-multiclass']) {
  test(`${eyewear}/${hair}: live switch, matched hold, stop and fresh restart`, async ({page}) => {
    ...
    await page.goto('/experiments/performance-candidate/live.html');
    ...
    const first = await collect(page, 'current'), candidate = await collect(page, 'test');
    ...
    for (const rows of [first, candidate]) expect(rows.some(row => {
    ...
    for (const key of ['surfacePositions', 'eyewearMatrix', 'protection'] as const)
      expect(diagnostic.current.captureSnapshot[key]).toEqual(diagnostic.test.captureSnapshot[key]);
    for (const pipeline of ['current', 'test', 'current', 'test'] as const) {
      await page.selectOption('#pipeline-select', pipeline);
    ...
    await writeFile(test.info().outputPath('receipt.json'), JSON.stringify({eyewear, hair, current: first, test: candidate,
...
test('cancel startup and restart without stale workers or images', async ({page}) => {
  ...
  await expect.poll(() => closed(page)).toBe(true);
  await page.selectOption('#pipeline-select', 'test'); await page.click('#start');
  await expect(page.locator('.stage')).toHaveAttribute('data-state', 'tracking');
  await expect(page.locator('.stage')).toHaveAttribute('data-pipeline', 'test');
