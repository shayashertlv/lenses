...
interface HeldComparison {current: HeldOutput; test: HeldOutput; selectedPipeline: Pipeline; candidateAccepted: boolean;}
interface PendingObservation {
  ...
  privateExportNull: boolean; holdRequested: boolean;
}
  ...
  await page.addInitScript(() => {
    ...
    setInterval(() => {
      ...
      if (state.armPendingSwitch && value.processing === true && stats === null && value.presented) {
        ...
        const requestedPipeline = preparedPipeline === 'current' ? 'test' : 'current';
...
async function waitCategory(page: Page): Promise<void> {
  await expect.poll(() => page.evaluate(() => {
    const value = window.hairLivePreview.diagnostics(), stats = value.stats as {hasMask?: boolean; maskOutputMode?: string} | null;
    return value.phase === 'live' && stats?.hasMask === true && stats.maskOutputMode === 'category-only';
  })).toBe(true);
}
...
async function inspectHairToggles(page: Page, comparison: HeldComparison): Promise<void> {
  ...
  for (const pipeline of ['current', 'test'] as const) {
    ...
    await expect(page.locator('#active-pipeline')).toHaveText(pipeline === 'current' ? 'Current algorithm' : 'Performance test');
...
test('pending private frame queues algorithm switch and Hold, then both held pipelines support exact hair toggles', async ({page}) => {
  const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
  await installCamera(page); await page.goto('/experiments/performance-candidate/live.html');
  await page.selectOption('#eyewear-select', 'tom-ford-clear'); await page.click('#start'); await waitCategory(page);
  ...
  expect(comparison.current.pair).toEqual(comparison.test.pair);
  expect(comparison.current.sourcePngDataUrl).toBe(comparison.test.sourcePngDataUrl);
  expect(comparison.current.mask.outputMode).toBe('full'); expect(comparison.test.mask.outputMode).toBe('full');
  expect(comparison.current.mask.categorySHA256).toBe(comparison.test.mask.categorySHA256);
  ...
  await writeFile(test.info().outputPath('pending-controls-receipt.json'), JSON.stringify({observation, pair: comparison.current.pair,
    exactHairToggles: true, liveCategoryObserved: true, resourcesClosed: true, errors,
...
test('failed held full-mask upgrade retains category pixels and unlocks exact pipeline toggles and download', async ({page}) => {
  const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
  await installCamera(page); await page.goto('/experiments/performance-candidate/live.html');
  await page.selectOption('#pipeline-select', 'test'); await page.selectOption('#eyewear-select', 'amber-horizon');
  await page.click('#start'); await waitCategory(page);
  ...
  expect(after.current.pair).toEqual(after.test.pair);
  expect(after.current.mask.categorySHA256).toBe(after.test.mask.categorySHA256);
  ...
  expect(saved.current.pair).toEqual(after.current.pair); expect(saved.test.mask).toEqual(after.test.mask);
  ...
  await writeFile(test.info().outputPath('failed-upgrade-controls-receipt.json'), JSON.stringify({pair: after.current.pair,
    failedUpgradePreservedCategoryPixels: true, liveCategoryObserved: true, togglesAndDownloadEnabled: true, resourcesClosed: true, errors,
