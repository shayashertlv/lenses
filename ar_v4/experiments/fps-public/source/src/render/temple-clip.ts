...
export function createTempleClip(root: Object3D,
  isOpticalLens: (material: Material) => boolean = material => material instanceof MeshPhysicalMaterial && material.transmission > 0) {
  ...
  root.traverse(object => {
    ...
    for (const material of materials) {
      if (owned.has(material) || isOpticalLens(material)) continue;
