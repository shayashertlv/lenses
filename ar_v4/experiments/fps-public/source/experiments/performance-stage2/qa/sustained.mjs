...
const base = option('base', 'http://127.0.0.1:8072').replace(/\/$/, ''), pagePath = option('page', '/experiments/performance-stage2/live.html');
...
const pipelines = option('pipelines', 'current,test,test2').split(','), width = Number(option('width', '960')), height = Number(option('height', '640'));
...
for (const id of pipelines) assert.ok(['current', 'test', 'test2'].includes(id));
...
async function scan(directory) {
  for (const entry of await fs.readdir(directory, {withFileTypes: true})) {
    if (['output', 'dist', 'node_modules', '.recovery', '.git'].includes(entry.name)) continue;
    const filename = path.join(directory, entry.name);
    if (entry.isDirectory()) await scan(filename);
    else if (/\.(ts|mjs|js|html|css|json|glb|tflite|task|wasm)$/.test(entry.name)) frozen.set(filename, sha(await fs.readFile(filename)));
  }
}
for (const directory of ['references', 'experiments/hair-live-preview', 'experiments/hair-arm-preview', 'experiments/performance-candidate', 'experiments/performance-stage2', 'public/models', 'public/mediapipe'])
...
if (args.includes('--preflight')) {
  console.log(JSON.stringify({preflight: true, frozenFiles: frozen.size, source: fixturePath,
...
const out = path.resolve('experiments/performance-stage2/qa/output', `sustained-${new Date().toISOString().replaceAll(':', '-')}`); await fs.mkdir(out, {recursive: true});
const report = {schema: 'ar-performance-stage2-sustained-synthetic-v1', complete: false, passed: false, createdAt: new Date().toISOString(), out, base, pagePath,
...
try {
  ...
  for (const eyewear of eyewears) for (const hair of hairModels) {
    ...
    await page.route('**/*', route => {
      ...
      if (url.pathname === '/qa-stage2-synthetic.png') return route.fulfill({contentType: 'image/png', body: fixtureBytes});
    ...
    await page.addInitScript(installSyntheticCamera, {fixtureUrl: '/qa-stage2-synthetic.png', width, height, requestedFps: 30});
    ...
    for (const pipeline of session.order) {
      ...
      console.log(JSON.stringify({event: 'measured', eyewear, hair, pipeline, ...segment.summary}));
    }
