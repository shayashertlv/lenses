...
type Pipeline = 'current' | 'test' | 'test2';
const PIPELINES = ['current', 'test', 'test2'] as const;
const PIPELINE_LABELS = {current: 'Long-hair checkpoint', test: 'Test 1', test2: 'Current · Test 2'} as const;
...
interface HeldComparison {current: HeldOutput; test: HeldOutput; test2: HeldOutput; selectedPipeline: Pipeline; candidateAccepted: boolean; schema: string;}
  ...
  await page.addInitScript(() => {
    ...
    setInterval(() => {
      ...
      if (state.armPendingSwitch && value.processing === true && stats === null && value.presented) {
        ...
        const requestedPipeline = preparedPipeline === 'current' ? 'test' : preparedPipeline === 'test' ? 'test2' : 'current';
        ...
        // variant is accepted. In particular Test2 must rebuild its skipped layers.
...
test('pending Test2 frame queues algorithm switch and Hold, then all three held pipelines support exact hair toggles', async ({page}) => {
  const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
  await installCamera(page); await page.goto('/experiments/performance-stage2/live.html');
  ...
  expect(observation.preparedPipeline).toBe('test2'); expect(observation.requestedPipeline).toBe('current');
  ...
  expect(comparison.schema).toBe('ar-performance-stage2-comparison-v1'); expect(comparison.selectedPipeline).toBe('test2');
...
test('failed held full-mask upgrade retains category pixels and unlocks exact pipeline toggles and download', async ({page}) => {
  const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
  await installCamera(page); await page.goto('/experiments/performance-stage2/live.html');
  await page.selectOption('#pipeline-select', 'test2'); await page.selectOption('#eyewear-select', 'amber-horizon');
  await page.click('#start'); await waitCategory(page, 'test2');
  ...
  expect(before.selectedPipeline).toBe('test2'); expect(after.selectedPipeline).toBe('test2');
  for (const pipeline of PIPELINES) {
  ...
  expect(saved.schema).toBe('ar-performance-stage2-comparison-v1');
  for (const pipeline of PIPELINES) {
  ...
  await writeFile(test.info().outputPath('failed-upgrade-controls-receipt.json'), JSON.stringify({pair: after.current.pair,
    failedUpgradePreservedCategoryPixels: true, liveCategoryObserved: ['test2'], exactHairToggles: [...PIPELINES], togglesAndDownloadEnabled: true, resourcesClosed: true, errors,
