...
import {checkProtocol, mechanismUse, noAuxiliaryCamera} from '../../speed-lab/qa/protocol.mjs';
import {FPS_OPTIONS} from '../runtime/options.ts';
export async function createMatchedStudy({currentModule, candidateModule, generated, profile, allowAsyncFallback = false, renderer}) {
  ...
  // Frozen fixtures carry real content hashes. The candidate's live ownership
  // field names changed; this adapter changes names only, never bytes/pose/mask.
  // Separate production lifecycle checks exercise session tokens and Hold hashing.
  const identityNames = (value, candidate) => {
    if (Array.isArray(value)) return value.map(item => identityNames(item, candidate));
    if (!value || typeof value !== 'object' || Object.getPrototypeOf(value) !== Object.prototype) return value;
    return Object.fromEntries(Object.entries(value).filter(([key]) => candidate || key !== 'identityKind').map(([key, item]) => {
      const rename = candidate ? {sourceSHA256: 'sourceIdentity', detectionSHA256: 'detectionIdentity'}
        : {sourceIdentity: 'sourceSHA256', detectionIdentity: 'detectionSHA256'};
      return [typeof item === 'string' ? rename[key] ?? key : key, identityNames(item, candidate)];
    }));
  };
  const CandidateRenderer = classes.candidate;
  classes.candidate = {async create(...args) {
    const instance = await CandidateRenderer.create(...args);
    return new Proxy(instance, {get(target, key) {
      const value = target[key];
      if (['stats', 'captureSnapshot'].includes(key)) return identityNames(value, false);
      if (typeof value !== 'function') return value;
      if (['present', 'prepare', 'finish'].includes(key)) return (...parameters) => value.apply(target, parameters.map(item => identityNames(item, true)));
      if (['copyHeldInput', 'exportDiagnostic'].includes(key)) return (...parameters) => identityNames(value.apply(target, parameters), false);
  ...
  const renderOne = async (name, input, pair, source, mask) => {
    ...
    async render(input) {
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        if (name === 'candidate' && FPS_OPTIONS.cpuCompose) {
          const cpu = stats?.candidatePerformance?.cpuComposition;
          diagnosticChecks[name].cpuCompositionRequested = cpu?.requested === true;
          diagnosticChecks[name].cpuCompositionUsed = cpu?.used === true;
          diagnosticChecks[name].cpuWordComparisonUsed = cpu?.wordComparisonUsed === true;
          diagnosticChecks[name].cpuFullReferenceAudit = cpu?.auditedReferencePixels === display.width * display.height;
          diagnosticChecks[name].cpuFullFinalAudit = cpu?.finalAuditPixels === display.width * display.height;
        }
        diagnosticChecks[name].pairedMaskApplied = stats?.hasMask === true && stats.maskOutputMode === 'category-only' && !stats.fallbackReason;
        Object.assign(diagnosticChecks[name], checkProtocol('candidate', options, stats, geometry, display.width, display.height, protocolPolicy));
