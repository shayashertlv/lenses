...
const candidateModule = option('candidate', '/experiments/fps-candidate/runtime/experiments/speed-lab/renderer.ts');
const profile = option('profile', 'combined');
assert.ok(['source','warm','copies','async','combined'].includes(profile), 'Unknown rendering profile.');
const profileOptions = PROFILES[profile].options;
const variant = option('variant', 'combined');
assert.ok(FPS_MODES.includes(variant) && variant !== 'g', 'Unknown candidate mode.');
...
await scan(path.join(workspace, 'experiments/fps-candidate/runtime'));
for (const file of ['experiments/fps-candidate/qa/matched.mjs', 'experiments/fps-candidate/qa/browser.mjs']) {
  const filename = path.join(workspace, file); runtime.set(filename, sha(await fs.readFile(filename)));
}
if (args.includes('--preflight')) {
  ...
  console.log(JSON.stringify({preflight: true, profile, profileOptions, allowAsyncFallback, basePreservation, frozenFilesVerified: frozen.size, runtimeFilesVerified: runtime.size, implementationLabels: {current: 'Accepted G Combined', candidate: `FPS candidate ${variant}`}, phases: plans.map(plan => ({id: plan.id, images: plan.sources.length, masks: plan.masks.length, cases: plan.cases.length}))}));
  process.exit(0);
}
const out = path.join(workspace, 'experiments/fps-candidate/qa/output', `matched-${new Date().toISOString().replaceAll(':', '-')}`);
await fs.mkdir(out, {recursive: true});
const report = {schema: diagnosticCase ? 'ar-fps-candidate-bounded-repeat-v1' : 'ar-fps-candidate-matched-v1', complete: false, passed: false, createdAt: new Date().toISOString(),
  base, currentModule, candidateModule, variant, profile, profileOptions, allowAsyncFallback, basePreservation, implementationLabels: {current: 'Accepted G Combined', candidate: `FPS candidate ${variant}`}, out, phases: [], runtimeHashes: Object.fromEntries(runtime),
  ...
  identityAdapter: 'The candidate receives the same verified content hashes through sourceIdentity/detectionIdentity. QA normalizes these string field names for archived comparisons. It does not construct or test live session-frame identities; production lifecycle tests cover those and held content hashing.',
  preservationBoundary: 'The current original G dependency bytes are verified against the manifest captured before this experiment. Historical Test2 manifest compatibility is not claimed for this already modified development checkout.',
  ...
  limits: ['No face or hair inference is rerun. Renderer wall times do not establish live FPS, face/hair contention, sustained mobile speed or physical-camera latency.',
    ...
    'The prior Test1 warmed current-renderer protected-pixel variance is retained and is not resolved by passing this later Base/Speed-lab still comparison.']};
const save = () => {
...
try {
  for (const plan of plans) {
    ...
    await page.goto(`${base}/qa-performance-pairs?fps=${variant}`);
    measured.environment = await page.evaluate(async parameters => {
      const {createMatchedStudy} = await import('/experiments/fps-candidate/qa/browser.mjs');
      ...
      const environment = {userAgent: navigator.userAgent, renderer: extension ? gl.getParameter(extension.UNMASKED_RENDERER_WEBGL) : null,
        candidateOptions: (await import('/experiments/fps-candidate/runtime/options.ts')).FPS_OPTIONS};
    ...
    assert.equal(measured.environment.candidateOptions.mode, variant);
    ...
    for (const required of ['/experiments/speed-lab/native/renderer.ts', '/experiments/fps-candidate/runtime/experiments/speed-lab/native/renderer.ts',
    ...
    if (profileOptions.asyncReadback) for (const required of ['/experiments/speed-lab/native/pbo-readback.ts',
      '/experiments/fps-candidate/runtime/experiments/speed-lab/native/pbo-readback.ts'])
      assert.ok(measured.moduleUrls.some(url => new URL(url).pathname === required), `Actual PBO dependency was not loaded: ${required}`);
