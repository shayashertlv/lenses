...
import {isRecord, sha256, validateHairRequest, validNonce} from './hair-protocol.ts';
import type {HairOutputMode, HairRawOutput, HairWorkerRequest, HairWorkerResponse} from './hair-protocol.ts';
  ...
  try {
    if (message.type === 'initialize') {
      ...
      post({type: 'ready', requestId: message.requestId, sessionNonce: message.sessionNonce, model: selected.id,
        ...
        initializationMs: performance.now() - start, delegate, outputMode});
    } else {
      ...
      segmenter.segment(message.image, result => {
        try {
          ...
          const common = {sourceIdentity: message.sourceIdentity, sequence: message.sequence, model: selected.id, modelSHA256: selected.sha256,
