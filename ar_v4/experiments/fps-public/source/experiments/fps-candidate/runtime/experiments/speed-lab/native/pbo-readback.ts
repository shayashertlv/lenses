...
export interface PboReadbackMetrics {
  ...
  reducedGlQueriesRequested: boolean; reducedGlQueriesUsed: boolean; stateValidationFallback: string | null;
  stateQueryCalls: number; errorCheckCalls: number;
  deferredSubmissionChecksUsed: boolean;
}
const empty = (): PboReadbackMetrics => ({queuedCalls: 0, queuedBytes: 0, retrievedCalls: 0, retrievedBytes: 0, polls: 0, waitMs: 0,
  ...
  reducedGlQueriesRequested: false, reducedGlQueriesUsed: false, stateValidationFallback: null, stateQueryCalls: 0, errorCheckCalls: 0,
  deferredSubmissionChecksUsed: false});
...
interface PackState {read: WebGLFramebuffer | null; pack: WebGLBuffer | null; values: number[];}
export interface PboReadbackOptions {
  /** Opt-in contract for the private native renderer context: this reader is the
   * only PBO user, no external code mutates PACK_* state, and ordinary Three
   * rendering ends on the default framebuffer. Three's async target-read API
   * must not run on this context. Canonical state is verified once before use;
   * an incompatible context keeps the conservative query-and-restore path. */
  reducedGlQueries?: boolean;
}
export interface PboCapturePlan {
  /** Explicit native draw plan. No same-context rendering or error-consuming
   * calls may occur after the final capture and before finish settles. Earlier
   * captures retain immediate validation, including before Three's camera draw:
   * Three shader-error reporting can consume GL errors. Omit for conservative
   * submission checks even when owned-state queries are enabled. */
  expectedCaptures?: 1 | 2;
}
...
export class PboNativeReadback {
  ...
  private count = 0;
  private expectedCaptures: 1 | 2 | null = null;
  private pending = false;
  private disposed = false;
  private measured = empty();
  private readonly reducedGlQueries: boolean;
  private ownedState: PackState | null | undefined;
  private stateValidationFallback: string | null = null;
  constructor(renderer: WebGLRenderer, options: PboReadbackOptions = {}) {
    this.renderer = renderer;
    const gl = renderer.getContext();
    if (!('fenceSync' in gl)) throw new Error('WebGL 2 pixel-pack buffers are unavailable.');
    this.gl = gl;
    this.reducedGlQueries = options.reducedGlQueries === true;
  }
  get metrics(): PboReadbackMetrics {return {...this.measured};}
  begin(width: number, height: number, plan: PboCapturePlan = {}): void {
    if (this.pending) throw new Error('A native readback pair is already pending.');
    this.cancel(); this.measured = {...empty(), reducedGlQueriesRequested: this.reducedGlQueries,
      reducedGlQueriesUsed: !!this.ownedState, stateValidationFallback: this.stateValidationFallback};
    ...
    if (plan.expectedCaptures !== undefined && plan.expectedCaptures !== 1 && plan.expectedCaptures !== 2)
      throw new Error('The native capture plan is invalid.');
    this.expectedCaptures = plan.expectedCaptures ?? null;
  ...
  capture(slot: 0 | 1): void {
    ...
    if (this.expectedCaptures !== null && slot >= this.expectedCaptures) throw new Error('The native capture plan is already complete.');
    ...
    try {this.withPackState(() => {
      ...
      this.measured.queuedCalls++; this.measured.queuedBytes += bytes;
      if (this.ownedState && this.expectedCaptures === slot + 1) this.measured.deferredSubmissionChecksUsed = true;
      else this.check('read submission');
      this.count++;
  ...
  async finish(isCurrent: () => boolean, timeoutMs = 500): Promise<{beauty: ImageData; camera: ImageData | null}> {
    ...
    if (this.expectedCaptures !== null && this.count !== this.expectedCaptures) throw new Error('The native capture plan is incomplete.');
    const gl = this.gl, generation = this.generation, started = performance.now(), measured = this.measured;
    const current = (): void => {
    ...
    try {
      ...
      gl.flush();
      if (!this.measured.deferredSubmissionChecksUsed) this.check('fence submission');
      for (;;) {
        current(); measured.polls++;
        const status = gl.clientWaitSync(this.sync, 0, 0);
        // WAIT_FAILED reports invalid sync/flags/timeout. Context/session checks
        // run before every poll and before retrieval. Submission and extraction
        // still check GL errors before publication. Only a planned final read
        // defers its submission/fence errors into the checked CPU retrieval;
        // no code on this private context may consume them in between.
        if (!this.ownedState) this.check('fence poll');
        if (status === gl.ALREADY_SIGNALED || status === gl.CONDITION_SATISFIED) break;
        if (status === gl.WAIT_FAILED) {
          if (this.ownedState) this.check('failed fence poll');
          throw new Error('The native readback fence failed.');
        }
  ...
  private queryPackState(): PackState {
    ...
    return {read, pack, values: this.packKeys().map(key => Number(gl.getParameter(key)))};
  }
  private withPackState<T>(operation: () => T): T {
    const gl = this.gl, keys = this.packKeys();
    let snapshot: PackState;
    if (this.reducedGlQueries && this.ownedState === undefined) {
      snapshot = this.queryPackState();
      this.ownedState = snapshot.read === null && snapshot.pack === null
        && snapshot.values.every((value, index) => value === (index === 0 ? 4 : 0)) ? snapshot : null;
      if (!this.ownedState) this.stateValidationFallback = 'Private native readback requires default framebuffer and canonical unbound pixel-pack state.';
      this.measured.reducedGlQueriesUsed = !!this.ownedState;
      this.measured.stateValidationFallback = this.stateValidationFallback;
    } else snapshot = this.ownedState ?? this.queryPackState();
    try {
      keys.forEach(key => gl.pixelStorei(key, key === gl.PACK_ALIGNMENT ? 1 : 0));
      return operation();
    } finally {
      // Three caches FRAMEBUFFER/DRAW together, but not their READ alias. The
      // raw bind is required even when its separate READ cache says "null".
      gl.bindFramebuffer(gl.READ_FRAMEBUFFER, snapshot.read); this.renderer.state.bindFramebuffer(gl.READ_FRAMEBUFFER, snapshot.read);
      gl.bindBuffer(gl.PIXEL_PACK_BUFFER, snapshot.pack);
      keys.forEach((key, index) => gl.pixelStorei(key, snapshot.values[index]!));
    }
  }
  cancel(): void {
