...
import type {SharedReadbackMetrics} from './shared-readback.ts';
    ...
    try {
      ...
      if (wantsCamera || asyncCapture) {
        try {
          ...
          if (asyncCapture) {
            this.pbo ??= new PboNativeReadback(this.renderer, {reducedGlQueries: FPS_OPTIONS.queries});
            // The optional clean render can compile a shader and consume GL errors.
            // Declare the plan so only the last read's validation may be deferred.
            this.pbo.begin(renderWidth, renderHeight, {expectedCaptures:wantsCamera&&!this.stages.speedLab.reuseSourcePixelsUsed?2:1});
          }
