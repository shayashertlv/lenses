...
import {isSourceIdentity, parseLiveIdentity} from '../../frame-identity.ts';
import type {HairModel, HairModelId} from './models.ts';
import {assertDimensions, assertSequence, assertSourceIdentity, hashHairMasks, isRecord, validateHairOutput, validateHairReady, validNonce} from './hair-protocol.ts';
...
export class HairClient {
  ...
  private liveSourceSession: string | null = null;
  ...
  async segment(image: ImageBitmap, sourceIdentity: string, sequence: number): Promise<HairSegmentationResult> {
    try {
      ...
      assertSourceIdentity(sourceIdentity); assertSequence(sequence); assertDimensions(image.width, image.height);
      if (!isSourceIdentity(sourceIdentity, sequence)) throw new Error('Hair source identity belongs to another frame sequence.');
      const liveSource = parseLiveIdentity(sourceIdentity);
      if (liveSource && this.liveSourceSession !== null && liveSource.sessionId !== this.liveSourceSession)
        throw new Error('Hair source identity belongs to another camera session.');
      if (liveSource) this.liveSourceSession = liveSource.sessionId;
      if (sequence <= this.lastSequence) throw new Error('Hair frame sequences must increase within a session.');
      this.lastSequence = sequence;
      const expected = {sourceIdentity, sequence, width: image.width, height: image.height, delegate: this.delegate, outputMode: this.outputMode};
      const result = await this.request({type: 'segment', sessionNonce: this.sessionNonce, requestId: this.nextId++, image, sourceIdentity, sequence},
