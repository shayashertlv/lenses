...
import {liveFrameIdentity, identityKind, heldContentPair} from '../../frame-identity.ts';
...
import type {FrameInput, FrameSample, ProfileSummary} from './frame-profiler.ts';
...
const profiler = new FrameProfiler(4096,FPS_OPTIONS.bookkeeping);
...
interface Session {
  ...
  cleanups: (() => void)[];
  nextSequence: number;
  presented: {sequence: number; sourceIdentity: string; detectionIdentity: string; capturedAtMs: number; pipeline: Pipeline} | null;
  heldAt: string | null;
  heldBusy: boolean;
  processing: boolean;
...
function showHairStatus(publishedStats?:LiveHairStats): void {
  ...
  if (session) {
    const fallback = FPS_OPTIONS.bookkeeping&&publishedStats ? publishedStats.fallbackReason : session.renderer?.stats?.fallbackReason;
    if (selectedVariant === 'accepted') text = 'Showing the accepted rendering. Toggle to compare the hair preview.';
    else if (session.hairError) text = session.phase === 'held' && (FPS_OPTIONS.bookkeeping&&publishedStats ? publishedStats.hasMask : session.renderer?.stats?.hasMask)
...
function updateProfileUi(session?: Session, publishedSummary?:ProfileSummary): void {
  ...
  const summary=FPS_OPTIONS.bookkeeping ? publishedSummary??profiler.liveSummary(session.id,session.performanceSample.pipeline,selectedVariant,performance.now())
    : summarize(profiler.recent(session.id,session.performanceSample.pipeline,selectedVariant));
...
function selectPipeline(pipeline: Pipeline, manual = true): void {
  profiler.invalidateLiveSummary();
...
function closeSession(message = 'Camera closed. Ready whenever you are.', failed = false): void {
  ...
  for (const key of ['frames', 'sessionId', 'frameSequence', 'sourceIdentity', 'detectionIdentity', 'identityKind', 'hairStatus', 'changedPixels']) delete stage.dataset[key];
...
function runPumpedFrames(session:Session):void {
  ...
  session.pump=runExperimentPump({id:session.id,video:session.camera!.video,renderer:session.renderer!,detector:session.detector,
    ...
    onPublished:(input,identity,stats)=>{
      if(!owns())return;const bookkeepingStarted=FPS_OPTIONS.bookkeeping?performance.now():0;session.performanceSample=profiler.add(input);
      ...
      stage.dataset.frameSequence=String(input.sequence);stage.dataset.sourceIdentity=identity.sourceIdentity;stage.dataset.detectionIdentity=identity.detectionIdentity;
      stage.dataset.identityKind=identityKind(identity.sourceIdentity);
      ...
      const summary=FPS_OPTIONS.bookkeeping ? profiler.liveSummary(session.id,input.pipeline,input.variant,performance.now())
        : summarize(profiler.recent(session.id,input.pipeline,input.variant));
      ...
      updateControls();showHairStatus(FPS_OPTIONS.bookkeeping?stats:undefined);updateProfileUi(session,summary);
      if(FPS_OPTIONS.bookkeeping)profiler.finishPublicationBookkeeping(session.performanceSample,performance.now()-bookkeepingStarted,1);
      if(session.holdRequested){holdSession(session);return;}
...
function runSerialFrames(session: Session): void {
  ...
  async function process(identity: number, metadata?: VideoFrameCallbackMetadata): Promise<void> {
    ...
    try {
      ...
      const sourceHashPromise = FPS_OPTIONS.identity ? Promise.resolve(liveFrameIdentity('source',session.id,sequence))
        : sha256(sourceBytes).then(hash => {sourceHashMs=performance.now()-sourceHashStarted;return hash;});
      ...
      const hairStartPromise = Promise.all([sourceHashPromise, hairBitmapPromise]).then(([sourceIdentity, bitmap]) => {
        if (!bitmap) return;
        ...
        const started = hairBudget.start(sequence, async () => {
          ...
          try {const work=session.hair.segment(bitmap,sourceIdentity,sequence);lastHairWork=work.catch(()=>null);return await work;}
          catch (error) {
      ...
      const [detection, sourceIdentity] = await Promise.all([facePromise, sourceHashPromise, hairStartPromise]);
      ...
      const detectionIdentity = FPS_OPTIONS.identity ? liveFrameIdentity('detection',session.id,sequence)
        : await sha256(new TextEncoder().encode(JSON.stringify(detection)));
      const detectionHashMs = FPS_OPTIONS.identity ? 0 : performance.now() - detectionHashStarted;
      if (!owns()) return;
      const pair = {sourceIdentity, detectionIdentity, eyewearModel: session.eyewearId};
      ...
      const ownedSource=createOwnedSourceFrame(capture,new ImageData(sourceBytes,width,height,{colorSpace:'srgb'}),
        {sourceIdentity,generation:sequence,sessionId:session.id,isCurrent:owns});
      ...
      if(FPS_OPTIONS.cpuCompose&&native)for(const [key,value] of Object.entries(native))if(key.startsWith('cpuComposition.'))native['fps.cpuCompose.'+key.slice('cpuComposition.'.length)]=value;
      if(FPS_OPTIONS.bookkeeping&&native)native['fps.bookkeeping.statsCollectionWorkMs']=performance.now()-publishedAtMs;
      const bookkeepingStarted=FPS_OPTIONS.bookkeeping?performance.now():0;
      ...
      session.presented = {sequence, sourceIdentity, detectionIdentity, capturedAtMs, pipeline: session.renderer!.pipeline};
      stage.dataset.frameSequence = String(sequence);
      stage.dataset.sourceIdentity = sourceIdentity; stage.dataset.detectionIdentity = detectionIdentity;
      stage.dataset.identityKind=identityKind(sourceIdentity);
      ...
      const summary=FPS_OPTIONS.bookkeeping?profiler.liveSummary(session.id,pipeline,selectedVariant,performance.now()):null;
      if(summary)element('stall-latency').textContent=summary.processing?`${Math.round(summary.processing.p95)} ms`:'—';
      else {
      ...
      updateControls(); showHairStatus(FPS_OPTIONS.bookkeeping?stats:undefined);
      updateProfileUi(session,summary??undefined);
      if(FPS_OPTIONS.bookkeeping)profiler.finishPublicationBookkeeping(session.performanceSample,performance.now()-bookkeepingStarted,1);
...
async function completeHeldHair(session: Session): Promise<void> {
  ...
  try {
    const heldContext=input.source.getContext('2d',{willReadFrequently:true,colorSpace:'srgb'});
    if(!heldContext)throw new Error('The retained held image is unavailable.');
    const pair=await heldContentPair(input.pair,heldContext.getImageData(0,0,input.source.width,input.source.height).data,input.detection);
    if(!owns())return;
    input.pair=pair;
    ...
    const result = await client.segment(bitmap, input.pair.sourceIdentity, session.presented!.sequence);
    if (!owns()) return;
    try {
      await session.renderer!.present(input.source, input.detection, {...result, detectionIdentity: input.pair.detectionIdentity}, input.pair, input.expectedModel);
    } catch (error) {
    ...
    if (owns()) {
      ...
      if(owns()){
        ...
        if(session.presented && typeof result?.sourceSHA256==='string' && typeof result.detectionSHA256==='string') {
          session.presented={...session.presented,sourceIdentity:result.sourceSHA256,detectionIdentity:result.detectionSHA256};
          stage.dataset.sourceIdentity=result.sourceSHA256;stage.dataset.detectionIdentity=result.detectionSHA256;
          stage.dataset.identityKind='sha256';
        }
...
function diagnostic(): Record<string, unknown> | null {
  ...
  return {...output, ...CURRENT_BASE_METADATA, liveSession: {id: session.id, phase: session.phase, heldAt: session.heldAt,
    presented: {...session.presented, identityKind:identityKind(session.presented.sourceIdentity),
      ...(identityKind(session.presented.sourceIdentity)==='sha256'?{sourceSHA256:session.presented.sourceIdentity,detectionSHA256:session.presented.detectionIdentity}:{})},
    sourceHashKind: identityKind(session.presented.sourceIdentity)==='sha256'?'SHA-256 of full captured RGBA bytes, unmirrored':null,
    detectionHashKind: identityKind(session.presented.sourceIdentity)==='sha256'?'SHA-256 of JSON.stringify of the exact validated Detection':null,
...
window.hairLivePreview = Object.freeze({
  diagnostics: () => ({...CURRENT_BASE_METADATA, state: stage.dataset.state, sessionId: current?.id ?? null, phase: current?.phase ?? null,
    presented: current?.presented ? {...current.presented,identityKind:identityKind(current.presented.sourceIdentity)} : null, hairReady: current?.hairReady ?? false,
