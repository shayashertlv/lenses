...
function assertIdentity(value: SourceFrameIdentity): void {
  const token = parseLiveIdentity(value.sourceIdentity);
  if (!isSourceIdentity(value.sourceIdentity, value.generation) || (token !== null && token.sessionId !== value.sessionId)
...
/** Claim the immutable getImageData snapshot under its session/frame or hash identity. Transferring the
...
export function createOwnedSourceFrame(canvas: HTMLCanvasElement, rgba: ImageData,
  ...
  return Object.freeze({canvas, rgba: owned, sourceIdentity: identity.sourceIdentity,
...
export function assertSourceFrameCurrent(source: OwnedSourceFrame, frame?: HTMLCanvasElement, sourceIdentity?: string): void {
  ...
  if (sourceIdentity !== undefined && source.sourceIdentity !== sourceIdentity)
