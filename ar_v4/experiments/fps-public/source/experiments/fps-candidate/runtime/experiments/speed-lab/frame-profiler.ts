...
import {PublicationWindow} from '../../bookkeeping.ts';
...
export class FrameProfiler {
  ...
  private readonly liveWindow:PublicationWindow|null;
  constructor(capacity = 4096, bookkeeping = false) {
    if (!Number.isInteger(capacity) || capacity<2) throw new Error('The profiling sample capacity must be at least two.');
    this.capacity=capacity;
    this.liveWindow=bookkeeping?new PublicationWindow(capacity):null;
  }
  add(input: FrameInput): FrameSample {
    ...
    const native=input.native ? Object.fromEntries(Object.entries(input.native).filter(([key,value])=>
      !/(^|\.)(sourceIdentity|detectionIdentity|\w*SHA256|\w*PngDataUrl|landmarks|categoryBase64|confidenceBase64)(\.|$)/i.test(key)
      // A future diagnostic rename must not make an explicit frame token leak
      // into timing-only exports under an otherwise harmless field name.
      && !(typeof value==='string' && /live:(?:source|detection):/i.test(value)))) : null;
    const row = {...input,native,serial:++this.serial};
    this.rows.push(row); if (this.rows.length>this.capacity) this.rows.shift();
    this.liveWindow?.add(row);
  ...
  progress(at: number): string {
    const run=this.benchmark;
    if (!run) return 'Select an experiment, then measure it against Test 2 for 30 seconds each after warmup.';
  ...
  recent(sessionId: string, pipeline: Pipeline, variant: string): FrameSample[] {
    ...
    return this.liveWindow?ordered.map(row=>({...row,native:row.native?{...row.native}:null})):ordered;
  }
  /** Readouts only. Every raw row is still available through samplesAfter and
   * exact benchmark/export summaries bypass this bounded percentile cache. */
  liveSummary(sessionId:string,pipeline:Pipeline,variant:string,at:number):ProfileSummary {
    if(!this.liveWindow)return summarize(this.recent(sessionId,pipeline,variant));
    return this.liveWindow.matches(sessionId,pipeline,variant)?this.liveWindow.summary(at,summarize):summarize([]);
  }
  invalidateLiveSummary():void {this.liveWindow?.invalidate();}
  /** Synchronous end of the publication callback, after its numeric/UI work.
   * This is diagnostic overhead after publishedAtMs, not render or image age. */
  finishPublicationBookkeeping(sample:FrameSample,elapsedMs:number,statsReadsAvoided:number):void {
    const row=this.rows.at(-1);
    if(!this.liveWindow||!row||row.serial!==sample.serial||row.sessionId!==sample.sessionId||row.sequence!==sample.sequence
      ||!this.liveWindow.matches(row.sessionId,row.pipeline,row.variant))throw new Error('Publication bookkeeping ownership expired.');
    if(!Number.isFinite(elapsedMs)||elapsedMs<0||!Number.isInteger(statsReadsAvoided)||statsReadsAvoided<0)throw new Error('Invalid publication bookkeeping measurement.');
    row.native={...row.native,'fps.bookkeeping.used':true,'fps.bookkeeping.publicationWorkMs':elapsedMs,
      'fps.bookkeeping.statsReadsAvoided':statsReadsAvoided,
      ...Object.fromEntries(Object.entries(this.liveWindow.counters).map(([key,value])=>['fps.bookkeeping.'+key,value]))};
    sample.native={...row.native};
  }
  snapshot(): Record<string, unknown> {
    const run=this.benchmark;
    return {schema:'ar-speed-lab-performance-v1',fpsCandidate:{mode:FPS_OPTIONS.mode,accepted:false,queriesRequested:FPS_OPTIONS.queries,liveFrameIdentity:FPS_OPTIONS.identity,
      cpuComposeRequested:FPS_OPTIONS.cpuCompose,bookkeepingRequested:FPS_OPTIONS.bookkeeping,humanSummaryCacheMs:FPS_OPTIONS.bookkeeping?500:0},...CURRENT_BASE_METADATA,baseCommit:'8baa16c9ffae945130b936821d5948c2f1cac720',timeOriginMs:performance.timeOrigin,
      ...
      schedulerPolicy:'schedulerWaitMs is finish-to-next-callback wait in serial modes, but capture-to-inference wait in fresh/overlap modes. These are different stages; compare frame intervals and totalMs across modes.',
