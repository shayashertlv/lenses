import {LiveHairRenderer as BaseRenderer} from '../performance-stage2/renderer.ts';
import {heldContentPair} from '../../frame-identity.ts';
import {LiveHairRenderer as CandidateRenderer} from './renderer.ts';
import {PIPELINES, PROFILES, usesBaseRenderer} from './profiles.ts';
...
/** Two renderer instances: unchanged Test 2 and one separately configured test.
 * Async preparation stays private. The chosen profile is frozen for each pair. */
export class ComparisonRenderer {
  private requested: Pipeline = 'base';
  private heldGeneration=0;
  private active: Pipeline = 'base';
  ...
  get captureSnapshot() {return this.pending || this.disposed ? null : this.renderer.captureSnapshot;}
  ...
  async prepare(frame: HTMLCanvasElement, detection: Detection, pair: PairIdentity, model: HairModelContract,
    needsHair=this.selectedVariant==='hair', source?: OwnedSourceFrame): Promise<boolean> {
    ...
    try {
      ...
      const visible=usesBaseRenderer(this.active) ? this.base.prepare(frame,detection,pair,model)
        : await this.candidate.prepare(frame,detection,pair,model,{options:PROFILES[this.active].options,source});
      if(this.disposed)throw new DOMException('Renderer closed.','AbortError');
  ...
  async present(frame: HTMLCanvasElement, detection: Detection, mask: HairMask|null,pair:PairIdentity,model:HairModelContract):Promise<boolean> {
    ...
    const source=createOwnedSourceFrame(frame,context.getImageData(0,0,frame.width,frame.height),{
      sourceIdentity:pair.sourceIdentity,generation:++this.heldGeneration,sessionId:'held',isCurrent:()=>!this.disposed});
    await this.prepare(frame,detection,pair,model,mask!==null || this.selectedVariant==='hair',source);return this.finish(mask);
  }
  ...
  async setHeld():Promise<void> {
    ...
    const selected=this.active, variant=this.selectedVariant;
    this.held=false;
    try {
      // Hash the exact retained pair once, before rendering any held comparison.
      // No partially upgraded pair or mask is ever published across this await.
      const ctx=input.source.getContext('2d',{willReadFrequently:true,colorSpace:'srgb'});
      if(!ctx)throw new Error('Missing held source context.');
      const pair=await heldContentPair(input.pair,ctx.getImageData(0,0,input.source.width,input.source.height).data,input.detection);
      if(this.disposed)throw new DOMException('Held comparison cancelled.','AbortError');
      const mask=this.mask?{...this.mask,sourceIdentity:pair.sourceIdentity,detectionIdentity:pair.detectionIdentity}:null;
      if(this.mask && (this.mask.sourceIdentity!==input.pair.sourceIdentity || this.mask.detectionIdentity!==input.pair.detectionIdentity))
        throw new Error('The retained hair mask belongs to another held image/detection pair.');
      input.pair=pair;
      for(const profile of PIPELINES) {
        if(this.disposed)return;
        if(profile==='fresh' || profile==='overlap') {nextOutputs.set(profile,nextOutputs.get('base')!);continue;}
        ...
        nextOutputs.set(profile,{diagnostic,accepted,hair});
      }
      this.cachedDiagnostic={schema:'ar-speed-lab-comparison-v1',baseCommit:'8baa16c9ffae945130b936821d5948c2f1cac720',
        identityKind:'sha256',sourceSHA256:pair.sourceSHA256,detectionSHA256:pair.detectionSHA256,
        candidateAccepted:false,inputPolicy:'Every output uses the same exact held source, detection and mask. Scheduler-only modes share base held pixels.',
