import {identityMetadata} from '../../frame-identity.ts';
import {TryOnRenderer as AcceptedRenderer} from '../../../../temple-sagittal/renderer.ts';
import type {CaptureGeometry as AcceptedGeometry} from '../../../../temple-sagittal/renderer.ts';
import {CleanCameraPass} from './background.ts';
...
import type {LiveHairArmInput as HairArmInput, HairMask} from './live-mask.ts';
...
import {copyHairMask, liveNasalRoi} from './ownership.ts';
import type {LiveVariant, OwnedPixels} from './ownership.ts';
import {composeHairArmsFast, checkHairProtection} from './fast-compose.ts';
import {loadTempleContinuityModel, projectTempleContinuity, findDetachedTemplePixels, TEMPLE_CONTINUITY_POLICY} from './continuity.ts';
import type {TempleContinuityModel, ContinuityDiagnostics} from './continuity.ts';
import {LONG_HAIR_REVISION} from './revision.ts';
...
export interface LiveHairTimings {
  acceptedRenderMs: number; acceptedReadbackMs: number; cleanCameraMs: number; composeMs: number;
...
export interface CaptureGeometry extends AcceptedGeometry {
  hairPreview: {method: string; variant: LiveVariant; applied: boolean; sourceIdentity: string; detectionIdentity: string;
...
/** Accepted owns its scene unchanged; hair is a separately cached late pixel copy. */
export class LiveHairRenderer {
  private readonly displayContext: CanvasRenderingContext2D;
  private readonly acceptedContext: CanvasRenderingContext2D;
  ...
  private camera: CleanCameraPass | null = null;
  ...
  private constructor(private readonly display: HTMLCanvasElement, private readonly acceptedCanvas: HTMLCanvasElement,
    ...
    this.displayContext = context(display); this.acceptedContext = context(acceptedCanvas);
  }
  ...
  static async create(display: HTMLCanvasElement, signal: AbortSignal, eyewearId: EyewearId = DEFAULT_EYEWEAR_ID): Promise<LiveHairRenderer> {
    ...
    try {
      accepted = await AcceptedRenderer.create(acceptedCanvas, signal, eyewearId);
  ...
  get captureSnapshot(): CaptureGeometry | null {
    ...
    return {...structuredClone(this.snapshot), hairPreview: {method: LIVE_HAIR_POLICY.method, variant: this.selected,
      ...
      sourceIdentity: this.pair.sourceIdentity, detectionIdentity: this.pair.detectionIdentity,
  ...
  prepare(frame: HTMLCanvasElement, detection: Detection, pair: PairIdentity, expectedModel: HairModelContract): boolean {
    ...
    try {
      ...
      const acceptedRenderMs = performance.now() - acceptedStarted, readbackStarted = performance.now();
      const {width, height} = this.acceptedCanvas;
      this.before = this.acceptedContext.getImageData(0, 0, width, height); this.after = this.before;
      const acceptedReadbackMs = performance.now() - readbackStarted;
      this.latestStats = {sequence: this.sequence, hasFace, hasMask: false, maskOutputMode: null, maskStatus: hasFace ? 'missing' : 'no-face',
        ...
        acceptedDiagnostics: null, continuity: null, timings: {acceptedRenderMs, acceptedReadbackMs, cleanCameraMs: 0, composeMs: 0,
  ...
  finish(mask: HairMask | null): boolean {
    ...
    try {
      ...
      try {
        ...
        this.camera ??= new CleanCameraPass();
        const clean = this.camera.present(this.source, width, height);
        this.latestStats.timings.cleanCameraMs = performance.now() - cleanStarted;
        if (clean.samples !== this.nativeSamples) throw new Error('The clean camera sampling differs from the accepted native renderer.');
        ...
        const input: HairArmInput = {width, height, before, background: clean.pixels, pair: ownedPair,
          geometryPair: {sourceIdentity: ownedPair.sourceIdentity, detectionIdentity: ownedPair.detectionIdentity, eyewearModel: this.eyewear.id},
          mask, expectedModel, protection, noseRoi: this.noseRoi};
        const composeStarted = performance.now(), result = composeHairArmsFast(input, {collectEligibleResidualIndices: !!this.continuityModel});
        ...
        // A lost/failed independent background may be recreated on a later frame.
  ...
  exportDiagnostic(): Record<string, unknown> | null {
    ...
    if (this.mask) {
      ...
      mask = {...identityMetadata(this.mask), sourceIdentity: this.mask.sourceIdentity, detectionIdentity: this.mask.detectionIdentity,
    ...
    return {schema: 'hair-live-diagnostic-v1', acceptedCommit: 'b26b5584c0dccbc2b30e4f12cdd432f10df577ea',
      acceptedRevisionLabel: LONG_HAIR_REVISION, hairAccepted: true,
      ...
      detection: structuredClone(this.detection), pair: {...this.pair, ...identityMetadata(this.pair)}, expectedModel: this.model ? structuredClone(this.model) : null,
      ...
      provenance: 'Exact current source RGBA and detection ownership supplied by the live session; genuine content hashes populated for held export. Semantic hair category heuristic with unchanged accepted optical/nasal guards; no measured hair depth or alpha.'};
  }
  ...
  dispose(): void {
    ...
    this.camera?.dispose(); this.camera = null; this.accepted.dispose(); this.clearOwned();
