...
import {isSourceIdentity} from '../../frame-identity.ts';
...
/** Root supplies detectionIdentity only after its same-frame face result is ready. */
...
export interface HairExpectedPair {sourceIdentity: string; sequence: number; width: number; height: number; delegate?: HairDelegate; outputMode?: HairOutputMode;}
...
export type HairWorkerRequest = Envelope & (
  ...
  | {type: 'segment'; image: ImageBitmap; sourceIdentity: string; sequence: number}
);
...
export function assertSourceIdentity(value: unknown): asserts value is string {
  check(isSourceIdentity(value), 'The hair source must have an explicit live ownership identity or genuine SHA-256 hash.');
}
...
export function validateHairOutput(value: unknown, model: HairModel, expected: HairExpectedPair): HairRawOutput {
  ...
  assertSourceIdentity(value.sourceIdentity); assertSequence(value.sequence); assertDimensions(value.width, value.height);
  check(isSourceIdentity(value.sourceIdentity, value.sequence), 'The hair source identity belongs to another frame sequence.');
  check(value.sourceIdentity === expected.sourceIdentity && value.sequence === expected.sequence,
  ...
  const common: HairOutputBase = {sourceIdentity: value.sourceIdentity, sequence: value.sequence, model: model.id, modelSHA256: model.sha256,
...
export function validateHairRequest(value: unknown): HairWorkerRequest {
  ...
  assertSourceIdentity(value.sourceIdentity); assertSequence(value.sequence); assertDimensions(value.image.width, value.image.height);
  check(isSourceIdentity(value.sourceIdentity, value.sequence), 'The hair source identity belongs to another frame sequence.');
  return {type: 'segment', sessionNonce: value.sessionNonce, requestId: value.requestId, image: value.image,
    sourceIdentity: value.sourceIdentity, sequence: value.sequence};
}
