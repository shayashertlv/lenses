...
function frame(at: number, pipeline:Pipeline='base', sequence=1): FrameInput {
  return {sessionId:'session',sequence,pipeline,variant:'hair',capturedAtMs:at-20,publishedAtMs:at,
    ...
    faceDelegate:'GPU',hairDelegate:'GPU',gpuRenderer:'source',cleanCameraMs:1,composeMs:1,continuityMs:0.5,
...
test('cadence uses completed publications, video delivery uses its own frame counter, overlapping stages stay distinct',()=>{
  const p=new FrameProfiler();p.add(frame(100,'base',1));p.add(frame(200,'base',2));p.add(frame(300,'base',3));
...
test('recent samples never cross pipeline or session changes and exports do not lend internal storage',()=>{
  const p=new FrameProfiler(3);p.add(frame(100));p.add(frame(200,'source'));p.add(frame(300));
  ...
  assert.equal(p.recent('session','base','hair').length,1);
  p.add({...frame(400),sessionId:'new'});assert.equal(p.samplesAfter(0)[0]!.serial,2);
  assert.equal(p.recent('new','base','hair').length,1);assert.equal(p.recent('session','base','hair').length,0);
  const output=JSON.parse(p.exportJSON()) as {currentBase:string;ownerSelectedG:boolean;previousBase:string;baseCommit:string};
  assert.equal(output.currentBase,'combined');assert.equal(output.ownerSelectedG,true);assert.equal(output.previousBase,'base');
  assert.equal(output.baseCommit,'8baa16c9ffae945130b936821d5948c2f1cac720');
});

test('benchmark requires warmup and thirty seconds per pipeline, drains switches once and retains complete numeric samples',()=>{
  const p=new FrameProfiler();assert.equal(p.begin('session','hair',0,['base','source','combined']),'base');
  let at=100;
  for(const pipeline of ['base','source','combined'] as const) {
    ...
    assert.equal(p.takeNextPipeline(),pipeline==='base'?'source':pipeline==='source'?'combined':null);
...
test('cancelled and changed-session measurements cannot silently resume or become complete',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['base','source','combined']);p.add(frame(100));p.add(frame(1000));p.add(frame(5100));p.cancel('Held');p.add(frame(40000));
  ...
  p.begin('session','hair',0,['base','source','combined']);p.add({...frame(50000),sessionId:'new'});
...
test('a measurement cannot claim warmup success without tracked hair frames',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['base','source','combined']);
...
test('a reversed two-mode comparison completes only its selected order and includes pending frame age',()=>{
  ...
  assert.equal(p.begin('session','hair',0,['overlap','base']),'overlap');
  completeSegment(p,'overlap',100,input=>({...input,capturedAtMs:input.publishedAtMs-100,totalMs:100,schedulerWaitMs:80}));
  assert.equal(p.takeNextPipeline(),'base');
  completeSegment(p,'base',36100,input=>input);
  assert.equal(p.running,false);assert.equal(p.takeNextPipeline(),null);
  const report=JSON.parse(p.exportJSON());
  assert.deepEqual(report.benchmark.order,['overlap','base']);
  ...
  assert.match(report.schedulerPolicy,/different stages/);
  assert.throws(()=>p.begin('session','hair',0,['base','base']),/Invalid comparison order/);
...
test('completed segments disclose lost tracking and missing masks without filtering faster fallback work',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['base','source','combined']);
  completeSegment(p,'base',100,input=>({...input,hasFace:false,hasMask:false,totalMs:2}));
  completeSegment(p,'source',36100,input=>({...input,hasMask:false,totalMs:5}));
...
test('hair-off measurements require tracking but do not report deliberately skipped masks as missing',()=>{
  const p=new FrameProfiler();p.begin('session','accepted',0,['base','source','combined']);
  let at=100;
  for(const pipeline of ['base','source','combined'] as const) {
...
test('completed comparison summaries remain cached and owned while live frames continue, and restart clears them',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['base','source','combined']);const revision=p.comparisonRevision;
  p.add(frame(100));assert.equal(p.comparisonRevision,revision);assert.equal(p.completedSegments.length,0);
  completeSegment(p,'base',200,input=>input);
