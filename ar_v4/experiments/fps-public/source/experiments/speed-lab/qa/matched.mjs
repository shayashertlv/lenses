...
const base = option('base', 'http://127.0.0.1:8082').replace(/\/$/, ''), phase = option('phase', 'all');
...
const candidateModule = option('candidate', '/experiments/speed-lab/renderer.ts');
...
if (args.includes('--preflight')) {
  ...
  console.log(JSON.stringify({preflight: true, profile, profileOptions, allowAsyncFallback, basePreservation, frozenFilesVerified: frozen.size, runtimeFilesVerified: runtime.size, implementationLabels: {current: 'Base Test2', candidate: `Speed lab ${profile}`}, phases: plans.map(plan => ({id: plan.id, images: plan.sources.length, masks: plan.masks.length, cases: plan.cases.length}))}));
  process.exit(0);
}
const out = path.join(workspace, 'experiments/speed-lab/qa/output', `matched-${new Date().toISOString().replaceAll(':', '-')}`);
await fs.mkdir(out, {recursive: true});
const report = {schema: diagnosticCase ? 'ar-speed-lab-bounded-repeat-v1' : 'ar-speed-lab-matched-v1', complete: false, passed: false, createdAt: new Date().toISOString(),
  base, currentModule, candidateModule, profile, profileOptions, allowAsyncFallback, basePreservation, implementationLabels: {current: 'Base Test2', candidate: `Speed lab ${profile}`}, out, phases: [], runtimeHashes: Object.fromEntries(runtime),
...
try {
  for (const plan of plans) {
    ...
    measured.environment = await page.evaluate(async parameters => {
      const {createMatchedStudy} = await import('/experiments/speed-lab/qa/browser.mjs');
    ...
    for (const required of ['/experiments/performance-stage2/native/renderer.ts', '/experiments/speed-lab/native/renderer.ts',
