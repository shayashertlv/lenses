...
import type {LiveHairArmInput as HairArmInput} from './live-mask.ts';
...
export function composeHairArmsFast(input: HairArmInput, options: {collectEligibleResidualIndices?: boolean} = {}): FastHairArmResult {
  ...
  const result: FastHairArmResult = {pixels: before.slice(), weights: new Float32Array(width * height), fallbackReason: null,
  ...
  try {
    ...
    for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
      ...
      result.weights[index] = weight; statistics.changedPixels++;
