  ...
  return {
    ...
    async render(input) {
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        check(diagnostic?.cleanBackgroundPngDataUrl && rawGeometry, `${name}: paired diagnostics absent.`);
    ...
    async controls(input) {
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        result[name] = {invalid, invalidRejected, noFaceCleared, restart, preparePrivate, overlappingRejected, finish, doubleFinishRejected};
      }
      result.passed = ['current', 'candidate'].every(name => {
        const row = result[name]; return row.invalid.changedPixels === 0 && row.invalidRejected && row.noFaceCleared && row.restart.changedPixels === 0
