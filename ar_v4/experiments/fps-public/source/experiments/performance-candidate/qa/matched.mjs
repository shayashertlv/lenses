...
const base = option('base', 'http://127.0.0.1:8066').replace(/\/$/, ''), phase = option('phase', 'all');
assert.equal(new URL(base).hostname, '127.0.0.1', 'QA attaches only to an explicit local server.');
const currentModule = option('current', '/experiments/hair-live-preview/renderer.ts');
const candidateModule = option('candidate', '/experiments/performance-candidate/renderer.ts');
const warmup = Number(option('warmup', '2')), samples = Number(option('samples', '5'));
...
for (const spec of specs) {
  ...
  for (const source of prior.images) {
  ...
  for (const mask of prior.masks) masks.push({...mask, categoryU8: await read(mask.categoryU8)});
  ...
  plans.push({...spec, priorReport: {path: filename, sha256: sha(bytes), bytes: bytes.length}, sources, masks, cases});
}
if (args.includes('--preflight')) {
  console.log(JSON.stringify({preflight: true, frozenFilesVerified: frozen.size, phases: plans.map(plan => ({id: plan.id, images: plan.sources.length, masks: plan.masks.length, cases: plan.cases.length}))}));
  process.exit(0);
}
...
const out = path.join(workspace, 'experiments/performance-candidate/qa/output', `matched-${new Date().toISOString().replaceAll(':', '-')}`);
await fs.mkdir(out, {recursive: true});
const report = {schema: diagnosticCase ? 'ar-performance-candidate-bounded-repeat-v1' : 'ar-performance-candidate-matched-v1', complete: false, passed: false, createdAt: new Date().toISOString(),
  base, currentModule, candidateModule, out, phases: [], runtimeHashes: Object.fromEntries(runtime), frozenInputs: [], errors: [],
  ...
  limits: ['No face or hair inference is rerun. Renderer wall times do not establish live FPS, face/hair contention, sustained mobile speed or physical-camera latency.',
    ...
    'The production live scheduler and current/test switching are checked separately.']};
...
try {
  for (const plan of plans) {
    ...
    measured.environment = await page.evaluate(async parameters => {
      const {createMatchedStudy} = await import('/experiments/performance-candidate/qa/browser.mjs');
    ...
    assert.ok(measured.moduleUrls.some(url => url.includes('/references/perfect-temples/') && url.includes('/src/render/renderer.ts')), 'Pinned accepted renderer was not loaded.');
