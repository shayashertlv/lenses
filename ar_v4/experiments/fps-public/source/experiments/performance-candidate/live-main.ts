...
import {DetectorClient} from '../../references/perfect-temples/src/runtime/detector.ts';
...
let selectedPipeline: Pipeline = 'current';
...
function updateControls(): void {
  ...
  element('active-pipeline').textContent = displayedPipeline === 'current' ? 'Current algorithm' : 'Performance test';
}
  ...
  async function process(identity: number): Promise<void> {
    ...
    try {
      ...
      setState(hasFace ? 'tracking' : 'searching', hasFace ? 'COMPARISON LIVE' : 'LOOKING FOR YOU',
        hasFace ? 'Switch between Current and Test to compare movement. Hold to compare the same image.' : 'Bring your face into view and look towards the camera.');
...
pipelineSelect.addEventListener('change', () => {
  selectedPipeline = pipelineSelect.value === 'test' ? 'test' : 'current';
  try { current?.renderer?.selectPipeline(selectedPipeline); updateControls(); showHairStatus(); }
...
for (const id of ['toggle-pipeline', 'stage-toggle-pipeline']) element(id).addEventListener('click', () => {
  ...
  pipelineSelect.value = selectedPipeline === 'current' ? 'test' : 'current';
