import {verifyRendererMechanism, verifyInactiveMechanism} from './gpu-contract.mjs';
/** Browser-only matched QA. Imports Test2 and Test3; never detects faces or hair. */
export async function createMatchedStudy({currentModule, candidateModule, generated}) {
  const classes = {
  ...
  const disposeSessions = () => {
    for (const session of Object.values(sessions)) { session.renderer.dispose(); session.abort.abort(); session.display.width = session.display.height = 0; }
    sessions = {};
  };
  const request = input => {
  ...
  const renderOne = (name, input, pair, source, mask) => {
    const session = sessions[name], started = performance.now();
    const visible = session.renderer.present(source.canvas, structuredClone(source.detection), mask, pair, input.expectedModel);
  ...
  return {
    ...
    async setModel(id) {
      ...
      try {
        for (const [name, Renderer] of Object.entries(classes)) {
          ...
          try { sessions[name] = {abort, display, renderer: await Renderer.create(display, abort.signal, id)}; }
    ...
    async render(input) {
      ...
      for (let iteration = 0; iteration < iterations; iteration++) {
        for (const name of iteration % 2 ? ['candidate', 'current'] : ['current', 'candidate']) {
          ...
          const sample = renderOne(name, input, pair, source, mask);
      ...
      const preExportStats = Object.fromEntries(Object.entries(sessions).map(([name, {renderer}]) => [name, structuredClone(renderer.stats)]));
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        diagnosticChecks[name] = {pair: equal(diagnostic.pair, pair), detection: equal(diagnostic.detection, source.detection),
          ...
          category: diagnostic.mask?.categoryBase64 ? await hash(Uint8Array.from(atob(diagnostic.mask.categoryBase64), c => c.charCodeAt(0))) === mask.categorySHA256 : false,
          geometryRawPose: equal(geometry.rawMatrix, source.detection.matrix)};
        const stats = renderer.stats;
        diagnosticChecks[name].maskAccepted = Boolean(stats?.hasMask && stats.maskOutputMode === 'category-only' && !stats.fallbackReason);
        if (name === 'current') {
          const native = stats.candidatePerformance?.nativePipeline?.native;
          diagnosticChecks[name].sharedNativeProtocol = native?.sharedCameraReady === true
            && native.sharedReadback?.readbackCalls === 2 && (native.packed?.copyCalls ?? 0) === 0;
        }
        if (name === 'candidate') diagnosticChecks[name].gpuMechanism = verifyRendererMechanism(preExportStats[name], input.expectedMechanism);
      ...
      return {id: input.id, eyewearModel: eyewear, hairModel: input.hairModel, pair, width: source.canvas.width, height: source.canvas.height,
        expectedMechanism: input.expectedMechanism, passed, checks, geometryChecks, diagnosticChecks, samples, pngs,
        current: {geometry: current.geometry, preExportStats: preExportStats.current, stats: current.stats, nativeSamples: current.nativeSamples,
          beforeRgbaSHA256: await hash(current.before), afterRgbaSHA256: await hash(current.after)},
        candidate: {geometry: candidate.geometry, preExportStats: preExportStats.candidate, stats: candidate.stats, nativeSamples: candidate.nativeSamples,
    ...
    async controls(input) {
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        renderer.selectVariant('hair'); renderOne(name, input, pair, source, mask); const good = pixels(display).slice();
        renderer.selectVariant('accepted'); const accepted = pixels(display).slice(); renderer.selectVariant('hair');
        renderer.selectVariant('accepted'); renderer.present(source.canvas, source.detection, null, pair, input.expectedModel);
        ...
        const hairOffNoAtlas = name === 'candidate' ? verifyInactiveMechanism(renderer.stats, 'hair-off') : hairOffNative?.sharedCameraReady === false
          && (hairOffNative.packed?.copyCalls ?? 0) === 0 && (hairOffNative.packed?.readbackCalls ?? 0) === 0
          && (hairOffNative.sharedReadback?.readbackCalls ?? 0) === 0;
        renderer.present(source.canvas, source.detection, mask, pair, input.expectedModel);
        ...
        renderer.present(source.canvas, source.detection, invalidMask, pair, input.expectedModel);
        ...
        const noFaceVisible = renderer.present(source.canvas, noFace, null, noFacePair, input.expectedModel);
        ...
        const noFaceNoAtlas = name === 'candidate' ? verifyInactiveMechanism(renderer.stats, 'no-face') : noFaceNative?.sharedCameraReady === false
          && (noFaceNative.packed?.copyCalls ?? 0) === 0 && (noFaceNative.packed?.readbackCalls ?? 0) === 0
          && (noFaceNative.sharedReadback?.readbackCalls ?? 0) === 0;
        renderOne(name, input, pair, source, mask); const restart = diff(good, pixels(display), display.width);
        const beforePrepare = pixels(display).slice(); renderer.prepare(source.canvas, source.detection, pair, input.expectedModel);
        ...
        let overlappingRejected = false; try { renderer.prepare(source.canvas, source.detection, pair, input.expectedModel); } catch { overlappingRejected = true; }
        ...
        result[name] = {hairOff, hairOffNative, hairOffNoAtlas, acceptedFirst, acceptedFirstHasMask, acceptedFirstHairToggle,
          ...
          restart, preparePrivate, overlappingRejected, finish, doubleFinishRejected};
      }
    ...
    async repeatDiagnostic(input) {
      ...
      for (let iteration = 0; iteration < input.repetitions; iteration++) {
        ...
        for (const name of iteration % 2 ? ['candidate', 'current'] : ['current', 'candidate']) {
          ...
          const timing = renderOne(name, input, pair, source, mask); check(timing.visible, 'Expected diagnostic face was not presented.');
    ...
    async cancellation(input) {
      ...
      for (const [name, session] of Object.entries(sessions)) {
        ...
        renderer.prepare(source.canvas, source.detection, pair, input.expectedModel); abort.abort(); renderer.dispose();
        ...
        result[name] = {closed, preAbortedRejected: rejected, canvasWidthAfterDisposal: display.width};
      }
      result.passed = ['current', 'candidate'].every(name => result[name].closed && result[name].preAbortedRejected); return result;
    },
