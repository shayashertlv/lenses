...
let selectedPipeline: Pipeline = 'test2';
...
function updateControls(): void {
  ...
  element('hold-hint').textContent = held
    ? 'Camera stopped. All four versions show this exact held image. Resume starts a fresh live session.'
...
function updateProfileUi(session?: Session): void {
  ...
  element('profile-gpu').textContent=summary.gpuHairFraction===null ? '—' : `${Math.round(summary.gpuHairFraction*100)}% of tracked frames`;
  const gpuUsed=session.performanceSample.native?.gpuCompositorUsed;
  element('profile-gpu-detail').textContent=session.performanceSample.pipeline!=='test3' ? ''
    : selectedVariant==='accepted' ? 'Hair is off.'
    : gpuUsed===true ? 'Test 3 GPU hair composition is active.'
    : session.performanceSample.hasMask ? 'CPU hair composition is active for this pose.'
    : 'This frame uses the accepted rendering; no hair composition was applied.';
...
function updateCompletedComparison(): void {
  ...
  for (const segment of completed) {
    ...
    const fields = {
      ...
      gpuHair: percent(segment.summary.gpuHairFraction),
    };
...
function runFrames(session: Session): void {
  ...
  async function process(identity: number, metadata?: VideoFrameCallbackMetadata): Promise<void> {
    ...
    try {
      ...
      session.renderer!.canvas.hidden = false; welcome.hidden = true;
...
async function openSession(): Promise<void> {
  ...
  for(const id of ['profile-fps','profile-video-fps','profile-p95','profile-interval-p95','profile-face','profile-face-roundtrip',
    'profile-scheduler','profile-copy','profile-prepare','profile-finish','profile-coverage','profile-gpu']) element(id).textContent='—';
  element('profile-gpu-detail').textContent='';
...
pipelineSelect.addEventListener('change', () => {
  ...
  selectPipeline(PIPELINES.includes(value as Pipeline) ? value as Pipeline : 'current');
});
...
download.addEventListener('click', () => {
  ...
  try {
    ...
    element('download-status').textContent = 'Comparison downloaded locally. It includes the original image and all four outputs.';
...
benchmark.addEventListener('click', () => {
  ...
  selectPipeline(profiler.begin(current.id,selectedVariant,performance.now()),false);
