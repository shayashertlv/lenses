...
export class TryOnRenderer {
  ...
  private gpuPair: GpuNativeFrame | null = null;
  ...
  /** GPU beauty is authoritative only while no separate rear-drop composition was required. */
  get gpuFrame(): GpuNativeFrame | null {return this.disposed ? null : this.gpuPair;}
  get stageTimings(): TempleStageTimings {
    if (!this.disposed && this.stages.native) this.stages.native = this.perfecto.stageTimings;
    return structuredClone(this.stages);
  }
  ...
  present(frame: HTMLCanvasElement, detection: Detection, recordedSurface?: readonly number[],
    ...
    try {
      ...
      const nativeGpu = this.perfecto.gpuFrame;
      const nativePair = this.perfecto.nativeImagePair;
      if (nativeGpu) {
        this.gpuPair = nativeGpu;
      } else if (nativePair) {
      ...
      try {
        ...
        if (dropM === 0) {
          ...
          // The separate nonzero branch remains the exact CPU reference path.
          // Its accepted pixels, not raw native beauty, must define hair permissions.
          if (this.gpuPair) {
            const readStarted = performance.now();
            this.baselinePixels = this.gpuPair.readDiagnostic('beauty');
            this.cameraPixels = this.gpuPair.cameraReady ? this.gpuPair.readDiagnostic('camera') : null;
            this.stages.baselineReadbackMs += performance.now() - readStarted;
            this.stages.baselineReadbackCalls += 1 + Number(this.cameraPixels !== null);
            this.stages.baselineReadbackBytes += this.baselinePixels.data.byteLength + (this.cameraPixels?.data.byteLength ?? 0);
            this.gpuPair = null;
          }
        ...
        const composed = dropM > 0 && this.branchPixels && this.baselinePixels ? composeProtectedPixels(this.baselinePixels.data, this.branchPixels.data, protection)
          : {pixels: this.baselinePixels?.data ?? null, changedPixels: 0, editablePixels: 0, protectedPixels: 0};
        if (!this.gpuPair && (!composed.pixels || !(composed.pixels.buffer instanceof ArrayBuffer))) throw new Error('Candidate output requires owned nonshared bytes.');
        this.candidatePixels = dropM === 0 ? this.baselinePixels
          : composed.pixels ? new ImageData(new Uint8ClampedArray(composed.pixels), width, height) : null;
        ...
        this.lastDiagnostics = {method: 'temple-optics-copy-v1', sequence: this.sequence, fallback: null,
          ...
          nativeReadback: this.gpuPair ? 'Test 3 same-size RGB8 GPU capture; no live colour readback' : 'Test 3 explicit CPU fallback',
  ...
  private publish(): void {
    ...
    if (this.publishes) {
      if (this.gpuPair) this.displayContext.drawImage(this.gpuPair.presentToCanvas('beauty'), 0, 0);
      else if (pixels) this.displayContext.putImageData(pixels, 0, 0);
    }
  ...
  exportDiagnostic(): Record<string, unknown> | null {
    if (this.gpuPair && !this.baselinePixels) {
      const readStarted = performance.now();
      this.baselinePixels = this.gpuPair.readDiagnostic('beauty');
      this.stages.baselineReadbackMs += performance.now() - readStarted;
      this.stages.baselineReadbackCalls++;
      this.stages.baselineReadbackBytes += this.baselinePixels.data.byteLength;
      this.candidatePixels = this.baselinePixels;
    }
    ...
    return {schema: 'ar_v4_sagittal_diagnostic_v1', exportedAt: new Date().toISOString(), sequence: this.sequence,
      ...
      baselineCommit: '31df28eb8ca0c698467fd9bfb16f737f9a74e915', acceptedRevisionLabel: 'performance Test 3; unaccepted',
  ...
  private clearOwnedPresentation(): void {
    this.perfecto.invalidateGpuFrame();
    this.gpuPair = null;
