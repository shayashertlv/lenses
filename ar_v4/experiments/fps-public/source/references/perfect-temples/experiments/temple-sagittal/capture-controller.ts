import { CaptureStore, CAPTURE_LIMITS } from '../../src/capture/store.ts';
import type { Detection } from '../../src/runtime/protocol.ts';
import { validateDetection } from '../../src/runtime/protocol.ts';
import type { TryOnRenderer } from './renderer.ts';
import type { CaptureGeometry } from './renderer.ts';
...
export class CaptureController {
  ...
  private begin(): void {
    ...
    this.reset();
    this.header = {
      renderer: 'temple-sagittal-comparison',
      comparison: { selectedVariantAtStart: renderer.variant, candidateAccepted: true,
        acceptedRevisionLabel: 'perfect_temples', baselineCommit: '31df28eb8ca0c698467fd9bfb16f737f9a74e915',
        acceptedCommit: '31df28eb8ca0c698467fd9bfb16f737f9a74e915',
        replay: 'actual per-frame rear drop, protection and original visibility settings',
        maximumRearDropMm: 20, measuredFit: false },
      nativeSamples: renderer.nativeSamples,
      ...
      templeVisibilityPolicy: 'Accepted perfecto side/frontal visibility. Each frame stores its actual weights and coverage; this candidate does not use the rejected contact-feather policy.',
      rearDropPolicy: 'Downward rear-temple lowering, maximum 20 mm, is an unmeasured preview convention. Frame metadata stores the actual rear drop and protected composition regions. Replay restores them without re-estimating the live policy.',
    };
    this.store.start();
  ...
  private async replay(): Promise<void> {
    ...
    try {
      ...
      renderer.present(this.image, validateDetection(recorded.detection),
        ...
        recorded.metadata?.templeVisibility ?? null, recorded.metadata?.rearDrop ?? null,
        recorded.metadata?.protection ?? null);
      const count = this.store.snapshot.frames.length;
