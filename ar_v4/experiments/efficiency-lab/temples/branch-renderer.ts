...
import {BranchLensOmission, emptyBranchLensMetrics} from './branch-lenses.ts';
...
export class TryOnRenderer {
  ...
  private lensOmission: BranchLensOmission | null = null;
  private lensMetrics = emptyBranchLensMetrics();
  ...
  get branchLensMetrics(): BranchLensMetrics {return {...this.lensMetrics};}
  ...
  private configure(face: CanonicalFace, eyewearScene: Group): void {
    ...
    // Classifiers above retain complete optical geometry. Visibility omission is
    // scoped only to a requested final branch draw, with unconditional restoration.
    this.lensOmission = new BranchLensOmission(eyewearScene);
  ...
  present(frame: HTMLCanvasElement, detection: Detection, recordedSurface?: readonly number[],
    ...
    recordedRearDrop?: RearDropConfiguration | null, omitLenses = false): boolean {
    ...
    try {
      ...
      this.lensMetrics = emptyBranchLensMetrics(omitLenses);
      if (this.lensOmission) this.lensMetrics = this.lensOmission.render(omitLenses && this.facePose.visible,
  ...
  private clearPresentation(resetGeometry = true): void {
    this.lensMetrics = emptyBranchLensMetrics();
