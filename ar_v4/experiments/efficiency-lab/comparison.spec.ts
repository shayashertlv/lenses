...
type Comparison = Record<typeof PIPELINES[number],Output> & {selectedPipeline:string;candidateAccepted:boolean;
  currentBase:string;ownerSelectedG:boolean;baseCommit:string;};
...
for (const eyewear of ['amber-horizon', 'tom-ford-clear']) for (const hair of ['hair-only', 'selfie-multiclass']) {
  test(`${eyewear}/${hair}: live switch, matched hold, stop and fresh restart`, async ({page}) => {
    ...
    expect(firstPublication.pipeline).toBe('g');
    expect(firstPublication.eyewearId).toBe(eyewear);expect(firstPublication.hairModelId).toBe(hair);
    ...
    expect(diagnostic.currentBase).toBe('g'); expect(diagnostic.ownerSelectedG).toBe(true); expect(diagnostic.baseCommit).toBe('b9142b2a3b957445f378d8012eea7e27ca68fd0b');
    for(const profile of PIPELINES){
      expect(diagnostic[profile].pair).toEqual(diagnostic.g.pair);
      expect(diagnostic[profile].sourcePngDataUrl).toBe(diagnostic.g.sourcePngDataUrl);
      expect(diagnostic[profile].mask).toEqual(diagnostic.g.mask);
      expect(diagnostic[profile].acceptedPngDataUrl).toBe(diagnostic.g.acceptedPngDataUrl);
      expect(diagnostic[profile].hairPngDataUrl).toBe(diagnostic.g.hairPngDataUrl);
      for(const key of ['surfacePositions','eyewearMatrix','protection'] as const)
        expect(diagnostic[profile].captureSnapshot[key]).toEqual(diagnostic.g.captureSnapshot[key]);
    ...
    for(const pipeline of PIPELINES){
      expect(metrics.samples.some(row=>row.pipeline===pipeline&&value(row,'reuseSourcePixelsUsed')===true)).toBe(true);
      expect(metrics.samples.some(row=>row.pipeline===pipeline&&value(row,'asyncReadbackUsed')===true)).toBe(true);
      expect(metrics.samples.some(row=>row.pipeline===pipeline&&value(row,'sourceCanvasBorrowed')===true)).toBe(true);
      expect(metrics.samples.some(row=>row.pipeline===pipeline&&value(row,'prewarmCompleted')===true)).toBe(true);
    }
    for(const pipeline of ['lean','combined']){
      const rows=metrics.samples.filter(row=>row.pipeline===pipeline);
      expect(rows.some(row=>Number(row.native?.['input.capturePool.reused'])>0)).toBe(true);
      expect(rows.every(row=>row.native?.['input.hashExplicitCopyBytes']===0)).toBe(true);
    }
    for(const pipeline of ['deferred','combined']){
      const rows=metrics.samples.filter(row=>row.pipeline===pipeline&&row.hasFace);
      expect(rows.some(row=>row.native?.['pump.deferPrefetch']===true&&typeof row.native?.['pump.nativeSubmittedMs']==='number')).toBe(true);
    }
    for(const pipeline of ['scratch','combined'])
      expect(metrics.samples.some(row=>row.pipeline===pipeline&&Number(value(row,'pbo.scratchBytesReused'))>0)).toBe(true);
    for(const pipeline of ['temples','combined'])
      expect(metrics.samples.some(row=>row.pipeline===pipeline&&value(row,'asyncTemplesUsed')===true)).toBe(true);
