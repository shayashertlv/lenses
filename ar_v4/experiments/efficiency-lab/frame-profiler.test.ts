...
function frame(at: number, pipeline:Pipeline='g', sequence=1): FrameInput {
  return {sessionId:'session',sequence,pipeline,variant:'hair',capturedAtMs:at-20,publishedAtMs:at,
    ...
    faceDelegate:'GPU',hairDelegate:'GPU',gpuRenderer:'scratch',cleanCameraMs:1,composeMs:1,continuityMs:0.5,
    finalChecksMs:0.5,publishMs:1,native:{readbacks:1}};
}

test('observation time exposes an open publication gap without inventing frames and retains fixed model IDs',()=>{
  const p=new FrameProfiler();assert.equal(p.snapshot(1000).lastPublication,null);
  p.add({...frame(800),eyewearId:'amber-horizon',hairModelId:'hair-only'});
  const before=p.samplesAfter(0);
  assert.deepEqual(p.snapshot(1000).lastPublication,{sessionId:'session',pipeline:'g',publishedAtMs:800,sinceLastPublicationMs:200});
  assert.deepEqual(p.snapshot(1400).lastPublication,{sessionId:'session',pipeline:'g',publishedAtMs:800,sinceLastPublicationMs:600});
  assert.equal(p.snapshot(1400).observedAtMs,1400);assert.deepEqual(p.samplesAfter(0),before);
  const row=JSON.parse(p.exportJSON()).samples[0];assert.equal(row.eyewearId,'amber-horizon');assert.equal(row.hairModelId,'hair-only');
  assert.deepEqual(p.snapshot(1400).events,[]);assert.throws(()=>p.snapshot(NaN),/observation time/);
});

test('cadence uses completed publications, video delivery uses its own frame counter, overlapping stages stay distinct',()=>{
  const p=new FrameProfiler();p.add(frame(100,'g',1));p.add(frame(200,'g',2));p.add(frame(300,'g',3));
...
test('publication stalls and capture age remain independent and terminal events survive without another frame',()=>{
  const p=new FrameProfiler();
  for(const [index,at] of [100,150,300,650,1250].entries())p.add(frame(at,'g',index+1));
  const summary=summarize(p.samplesAfter(0));
  assert.equal(summary.processing?.p95,20);
  assert.equal(summary.frameInterval?.max,600);
  assert.deepEqual(summary.gapCounts,{over100Ms:3,over200Ms:2,over500Ms:1});
  p.recordEvent('session','session-failed',2000);
  const events=p.snapshot().events as {name:string;atMs:number}[];
  assert.deepEqual(events,[{sessionId:'session',name:'session-failed',atMs:2000,durationMs:null}]);
  events[0]!.atMs=0;
  assert.equal((p.snapshot().events as {atMs:number}[])[0]!.atMs,2000);
  assert.throws(()=>p.recordEvent('session','frame-held',NaN),/Invalid timing event/);
});

test('recent samples never cross pipeline or session changes and exports do not lend internal storage',()=>{
  const p=new FrameProfiler(3);p.add(frame(100));p.add(frame(200,'scratch'));p.add(frame(300));
  const rows=p.samplesAfter(0);rows[0]!.native!.readbacks=99;rows[0]!.pipeline='combined';
  assert.equal(p.samplesAfter(0)[0]!.native!.readbacks,1);
  assert.equal(p.recent('session','g','hair').length,1);
  p.add({...frame(400),sessionId:'new'});assert.equal(p.samplesAfter(0)[0]!.serial,2);
  assert.equal(p.recent('new','g','hair').length,1);assert.equal(p.recent('session','g','hair').length,0);
  const output=JSON.parse(p.exportJSON()) as {currentBase:string;ownerSelectedG:boolean;previousBase:string;baseCommit:string};
  assert.equal(output.currentBase,'g');assert.equal(output.ownerSelectedG,true);
...
test('benchmark requires warmup and thirty seconds per pipeline, drains switches once and retains complete numeric samples',()=>{
  ...
  let at=100;
  for(const pipeline of ['g','scratch','combined'] as const) {
    ...
    assert.equal(p.takeNextPipeline(),pipeline==='g'?'scratch':pipeline==='scratch'?'combined':null);
  ...
  assert.deepEqual(run.segments.map(s=>s.samples.length),[3,3,3]);
  assert.deepEqual(run.segments.map(s=>s.summary.durationMs),[30000,30000,30000]);
...
test('cancelled and changed-session measurements cannot silently resume or become complete',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['g','scratch','combined']);p.add(frame(100));p.add(frame(1000));p.add(frame(5100));p.cancel('Held');p.add(frame(40000));
  ...
  p.begin('session','hair',0,['g','scratch','combined']);p.add({...frame(50000),sessionId:'new'});
...
test('a measurement cannot claim warmup success without tracked hair frames',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['g','scratch','combined']);
...
test('a reversed two-mode comparison completes only its selected order and includes pending frame age',()=>{
  const p=new FrameProfiler();
  assert.equal(p.begin('session','hair',0,['deferred','g']),'deferred');
  completeSegment(p,'deferred',100,input=>({...input,capturedAtMs:input.publishedAtMs-100,totalMs:100,schedulerWaitMs:80}));
  assert.equal(p.takeNextPipeline(),'g');
  completeSegment(p,'g',36100,input=>input);
  assert.equal(p.running,false);assert.equal(p.takeNextPipeline(),null);
  const report=JSON.parse(p.exportJSON());
  assert.deepEqual(report.benchmark.order,['deferred','g']);
  assert.deepEqual(report.benchmark.segments.map((segment:{summary:{processing:{median:number}}})=>segment.summary.processing.median),[100,20]);
  assert.match(report.schedulerPolicy,/capture-to-inference/);
  assert.throws(()=>p.begin('session','hair',0,['g','g']),/Invalid comparison order/);
  assert.throws(()=>p.begin('session','hair',0,[]),/Invalid comparison order/);
});

test('image ownership identifiers never enter timing storage or exports',()=>{
  const p=new FrameProfiler();
  const sample=p.add({...frame(100),native:{'speedLab.sourceIdentity.sourceSHA256':'private-image-hash',
    'speedLab.sourceIdentity.sessionId':'owned-image-session','detectionSHA256':'private-pose-hash',
    'native.pbo.retrievedCalls':1,'native.speedLab.asyncReadbackUsed':true}});
  assert.deepEqual(sample.native,{'native.pbo.retrievedCalls':1,'native.speedLab.asyncReadbackUsed':true});
  assert.doesNotMatch(p.exportJSON(),/private-image-hash|private-pose-hash|sourceIdentity|sourceSHA256|detectionSHA256/);
  assert.deepEqual(p.samplesAfter(0)[0]?.native,sample.native);
});

test('completed segments disclose lost tracking and missing masks without filtering faster fallback work',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['g','scratch','combined']);
  completeSegment(p,'g',100,input=>({...input,hasFace:false,hasMask:false,totalMs:2}));
  completeSegment(p,'scratch',36100,input=>({...input,hasMask:false,totalMs:5}));
  completeSegment(p,'combined',72100,input=>input);
  const completed=p.completedSegments;
  assert.deepEqual(completed.map(segment=>segment.coverage.status),['no-tracking','partial','full']);
  assert.deepEqual(completed.map(segment=>segment.summary.frames),[3,3,3]);
  assert.deepEqual(completed.map(segment=>segment.summary.processing?.median),[2,5,20]);
  ...
  assert.deepEqual(report.benchmark.segments.map(segment=>segment.coverage.status),['no-tracking','partial','full']);
  assert.deepEqual(report.benchmark.segments.map(segment=>segment.samples.length),[3,3,3]);
});
...
test('hair-off measurements require tracking but do not report deliberately skipped masks as missing',()=>{
  const p=new FrameProfiler();p.begin('session','accepted',0,['g','scratch','combined']);
  let at=100;
  for(const pipeline of ['g','scratch','combined'] as const) {
...
test('completed comparison summaries remain cached and owned while live frames continue, and restart clears them',()=>{
  const p=new FrameProfiler();p.begin('session','hair',0,['g','scratch','combined']);const revision=p.comparisonRevision;
  p.add(frame(100));assert.equal(p.comparisonRevision,revision);assert.equal(p.completedSegments.length,0);
  completeSegment(p,'g',200,input=>input);
