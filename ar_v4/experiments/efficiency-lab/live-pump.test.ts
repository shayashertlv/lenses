...
function browserFixture() {
  ...
  replace('document', {createElement: () => {
    const canvas = {width: 300, height: 150, getContext: () => ({
      drawImage(source: unknown) {if(source === video) {videoDraws++; onVideoDraw();}}, clearRect() {}, getImageData: () => {
        readbacks++;
        const data = new Uint8ClampedArray(canvas.width * canvas.height * 4);
        for (let i = 3; i < data.length; i += 4) data[i] = 255;
        return new Pixels(data, canvas.width, canvas.height);
      },
  ...
  const offer = (time: number): void => {
    video.currentTime = time; const current = callback; callback = undefined;
    assert.ok(current); current(performance.now(), {mediaTime: time, presentedFrames: time, presentationTime: performance.now()} as VideoFrameCallbackMetadata);
  };
  const restore = (): void => {for (const [key, descriptor] of saved) {
    if (descriptor) Object.defineProperty(globalThis, key, descriptor); else Reflect.deleteProperty(globalThis, key);
  }};
  return {replace, canvases, video, offer, restore, counts: () => ({videoDraws, readbacks}),
...
test('a video change during snapshot consumes no capture admission', async () => {
  const f = browserFixture(); let clock = 1000; f.replace('performance', {now: () => clock});
  const h = rateFixture(f, 'rate8');
  try {
    f.onVideoDraw(() => {f.video.currentTime += 0.01;}); f.offer(1);
    assert.equal(h.stats().admission.captures, 0); assert.equal(h.detectorCalls(), 0);
    assert.equal(f.counts().readbacks, 0); f.onVideoDraw(() => {});
    clock++; f.offer(2); await h.published(1);
    assert.equal(h.rows[0]!.capturedAtMs, 1001); assert.equal(h.stats().admission.captures, 1);
    assert.deepEqual(h.errors, []);
  } finally {h.pump.stop(); await h.pump.finishCurrent(); f.restore();}
});
...
test('Stop revokes pending work and a restarted capped session admits its first image immediately', async () => {
  ...
  try {
    f.offer(1); await detected.promise; old.pump.stop(); clock = 1001;
