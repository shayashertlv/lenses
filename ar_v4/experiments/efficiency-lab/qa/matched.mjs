...
import {summarizeMechanisms} from './protocol.mjs';
...
if (args.includes('--preflight')) {
  ...
  console.log(JSON.stringify({preflight: true, profile, profileOptions, allowAsyncFallback, basePreservation, frozenFilesVerified: frozen.size, runtimeFilesVerified: runtime.size, implementationLabels: {current: 'Current G Combined / b9142b2', candidate: `Efficiency lab ${profile}`}, phases: plans.map(plan => ({id: plan.id, images: plan.sources.length, masks: plan.masks.length, cases: plan.cases.length}))}));
  process.exit(0);
}
const out = path.join(workspace, 'experiments/efficiency-lab/qa/output', `matched-${new Date().toISOString().replaceAll(':', '-')}`);
...
try {
  for (const plan of plans) {
    ...
    measured.environment = await page.evaluate(async parameters => {
      const {createMatchedStudy} = await import('/experiments/efficiency-lab/qa/browser.mjs');
    ...
    for (const required of ['/experiments/speed-lab/native/renderer.ts', '/experiments/efficiency-lab/native/renderer.ts',
    ...
    if (profileOptions.asyncReadback) assert.ok(measured.moduleUrls.some(url => new URL(url).pathname === '/experiments/speed-lab/native/pbo-readback.ts'));
