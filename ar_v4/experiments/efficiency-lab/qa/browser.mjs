...
import {createOwnedSourceFrame} from '../../speed-lab/speed-options.ts';
...
export async function createMatchedStudy({currentModule, candidateModule, generated, profile, allowAsyncFallback = false, renderer}) {
  ...
  const frameInput = (name, source, pair, canvas = source.canvas) => {
    const frameOptions = name === 'current' ? G_PROFILES.combined.options : options;
    ...
    session.frameInput = {options: frameOptions, source: createOwnedSourceFrame(canvas, rgba, {
  ...
  const renderOne = async (name, input, pair, source, mask) => {
    ...
    async render(input) {
      ...
      const captures = {}, checks = {}, geometryChecks = {}, diagnosticChecks = {}, pngs = {}, rawBranches = {};
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        Object.assign(diagnosticChecks[name], checkProtocol(name, name === 'current' ? G_PROFILES.combined.options : options, stats, geometry, display.width, display.height, protocolPolicy));
        ...
        if (options.omitBranchLenses) {
          // QA-only access to the existing lower diagnostic export. Never read
          // a discarded default framebuffer or add raw downloads to live work.
          const lower = Reflect.get(renderer, 'accepted');
          check(lower && typeof lower.exportDiagnostic === 'function', 'Missing paired raw branch diagnostic.');
          const lowerDiagnostic = lower.exportDiagnostic();
          check(lowerDiagnostic && equal(lowerDiagnostic.protection, protection), 'Raw branch protection differs from the exact pair.');
          if (lowerDiagnostic.branchPngDataUrl) {
            const branchCanvas = await decodeURL(lowerDiagnostic.branchPngDataUrl);
            check(branchCanvas.width === display.width && branchCanvas.height === display.height, 'Raw branch dimensions differ.');
            rawBranches[name] = pixels(branchCanvas).slice(); pngs[`${name}Branch`] = lowerDiagnostic.branchPngDataUrl;
            branchCanvas.width = branchCanvas.height = 0;
          } else rawBranches[name] = null;
        }
      ...
      let rawBranch;
      if (options.omitBranchLenses) {
        const expected = rawBranches.current, actual = rawBranches.candidate;
        check(Boolean(expected) === Boolean(actual), 'Candidate lost or fabricated a paired raw branch.');
        check(Boolean(expected) === (candidate.geometry.rearDrop.dropM !== 0), 'Raw branch presence differs from required rear drop.');
        if (expected && actual) {
          const protection = candidate.geometry.protection;
          checks.rawBranchOutsideProtected = diff(expected, actual, source.canvas.width,
            (x, y) => !protection.protectedRects.some(rect => inside(rect, x, y)));
          rawBranch = {compared: true, whole: diff(expected, actual, source.canvas.width),
            outsideProtected: checks.rawBranchOutsideProtected, currentRgbaSHA256: await hash(expected),
            candidateRgbaSHA256: await hash(actual)};
        } else rawBranch = {compared: false, reason: 'Exact zero rear drop; no branch required.'};
      }
      ...
      return {id: input.id, eyewearModel: eyewear, hairModel: input.hairModel, pair, width: source.canvas.width, height: source.canvas.height,
        passed, profile, options, checks, geometryChecks, diagnosticChecks, samples, pngs, ...(rawBranch ? {rawBranch} : {}),
    ...
    async regionDiagnostic(input) {
      check(options.cropBranchReadback, 'Region diagnostic control requires the actual R profile.');
      const {source, mask, pair} = request(input), {renderer, display} = sessions.candidate;
      await presentOwned('current', source, source.detection, mask, pair, input.expectedModel);
      const expectedRaw = Reflect.get(sessions.current.renderer, 'accepted').exportDiagnostic();
      check(expectedRaw?.branchPngDataUrl, 'Region ownership control needs a real nonzero G branch.');
      const borrowed = document.createElement('canvas'); borrowed.width = source.canvas.width; borrowed.height = source.canvas.height;
      borrowed.getContext('2d', {willReadFrequently:true}).putImageData(
        new ImageData(pixels(source.canvas).slice(), borrowed.width, borrowed.height), 0, 0);
      const owned = frameInput('candidate', source, pair, borrowed);
      renderer.selectVariant('hair');
      await renderer.present(borrowed, structuredClone(source.detection), mask, pair, input.expectedModel, owned);
      const liveStats = structuredClone(renderer.stats), liveDisplaySHA256 = await hash(pixels(display));
      const liveGeometry = renderer.captureSnapshot;
      check(liveStats.candidatePerformance.nativePipeline.efficiencyLab.branchRegion.used, 'The diagnostic control did not first exercise R.');
      sessions.candidate.lease.current = false; borrowed.width = borrowed.height = 0;
      await new Promise(resolve => setTimeout(resolve, 0));
      const diagnostic = Reflect.get(renderer, 'accepted').exportDiagnostic();
      check(diagnostic?.branchPngDataUrl, 'Retained R source could not produce a full raw branch.');
      const expectedCanvas = await decodeURL(expectedRaw.branchPngDataUrl), actualCanvas = await decodeURL(diagnostic.branchPngDataUrl);
      const rawFull = diff(pixels(expectedCanvas), pixels(actualCanvas), actualCanvas.width);
      const checks = {source: diagnostic.sourcePngDataUrl === source.canvas.toDataURL('image/png'),
        detection: equal(diagnostic.detection, source.detection), geometry: ['eyewearModelId','rawMatrix','correctedMatrix','eyewearMatrix',
          'surfacePositions','yawDegrees','occlusion','templeClip','templeVisibility','rearDrop','protection']
          .every(key => equal(diagnostic.snapshot[key], liveGeometry[key])),
        dimensions: actualCanvas.width === source.canvas.width && actualCanvas.height === source.canvas.height,
        displayUnchanged: await hash(pixels(display)) === liveDisplaySHA256,
        statsUnchanged: equal(renderer.stats, liveStats), rawBranchExact: rawFull.changedPixels === 0,
        explicitFullRead: diagnostic.diagnosticBranchReadback?.rerendered === true && diagnostic.diagnosticBranchReadback.calls === 1
          && diagnostic.diagnosticBranchReadback.bytes === actualCanvas.width * actualCanvas.height * 4
          && diagnostic.diagnosticBranchReadback.source === 'same-pair-full-rerender'};
      const result = {passed:Object.values(checks).every(Boolean),checks,liveStats,liveDisplaySHA256,rawFull,
        rawBranchSHA256:await hash(pixels(actualCanvas)),diagnosticReadback:diagnostic.diagnosticBranchReadback,
        sourceLeaseRevoked:true,borrowedCanvasCleared:true,scope:'Explicit diagnostic after a task boundary; separate from saved live case readback totals.'};
      expectedCanvas.width=expectedCanvas.height=actualCanvas.width=actualCanvas.height=0;return result;
    },
    async controls(input) {
      ...
      for (const [name, {renderer, display}] of Object.entries(sessions)) {
        ...
        if (options.reuseSourcePixels || options.fewerCopies) {
          if (sessions[name].lease) sessions[name].lease.current = false;
          await renderer.present(source.canvas, source.detection, mask, pair, input.expectedModel, {options: name === 'current' ? G_PROFILES.combined.options : options});
