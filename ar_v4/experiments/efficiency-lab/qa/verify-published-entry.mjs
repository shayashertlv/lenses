...
try {
  ...
  async function inspect(label, study, options, selected = 'g') {
    ...
    assert.equal(await page.locator('.study-links a').count(), 2);
    ...
    const observation = {label, url: page.url(), options, selected, title: await page.title(),
      measureButton: await page.locator('#continuous-start').textContent(),
  ...
  await assertNoCamera();
  await entry.click();
  await inspect('Website landing → ar_testing', 'fps-review', expectedOptions);
  ...
  await inspect('G / V → All FPS experiments', 'fps-review', expectedOptions);
  await page.goto(base.origin + livePath);
  await inspect('Query-free AR bookmark', 'fps-review', expectedOptions);
  ...
  process.stdout.write('Verified landing → all six FPS experiments and G / V navigation; release '
