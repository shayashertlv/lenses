...
import {observeGlErrors} from '../../speed-lab/qa/gl-errors.mjs';
import {verifyBase} from './preservation.mjs';
import {PROFILES} from '../../speed-lab/profiles.ts';
import {summarizeMechanisms} from '../../speed-lab/qa/protocol.mjs';

const args = process.argv.slice(2);
const option = (name, fallback) => args.find(arg => arg.startsWith(`--${name}=`))?.slice(name.length + 3) ?? fallback;
const base = option('base', 'http://127.0.0.1:8094').replace(/\/$/, ''), phase = option('phase', 'all');
...
const candidateModule = option('candidate', '/experiments/efficiency-lab/g-readback-diagnostic/renderer.ts');
const profile = 'combined'; // Exact G options on both paths; never efficiency-lab's combined candidate.
...
for (const spec of specs) {
  ...
  for (const row of prior.cases.filter(row => !selectedSource || row.id === selectedSource)) {
    ...
    cases.push({id: row.id, eyewearModel: row.eyewearModel, hairModel: row.hairModel,
      ...
      expectedGeometry, noseRoi: spec.generated ? geometry.noseRoi : geometry.hairPreview.noseRoi,
...
async function scan(directory) {
  for (const entry of await fs.readdir(directory, {withFileTypes: true})) {
    if (['output', 'node_modules', '.recovery', 'dist', '.git', 'test-results', 'logs'].includes(entry.name)) continue;
    const filename = path.join(directory, entry.name);
    if (entry.isDirectory()) await scan(filename);
    else if (/\.(ts|mjs|py|html|css|json|glb|tflite|task|wasm|js)$/.test(entry.name)) runtime.set(filename, sha(await fs.readFile(filename)));
  }
}
...
for (const directory of ['native', 'temples', 'qa']) await scan(path.join(workspace, 'experiments/speed-lab', directory));
for (const file of ['renderer.ts', 'speed-options.ts', 'profiles.ts', 'vite.config.ts']) {
  const filename = path.join(workspace, 'experiments/speed-lab', file); runtime.set(filename, sha(await fs.readFile(filename)));
}
await scan(path.join(workspace, 'experiments/efficiency-lab'));
if (args.includes('--preflight')) {
  for (const required of ['renderer.ts', 'vite.config.ts']) await fs.access(path.join(workspace, 'experiments/performance-stage2', required));
  console.log(JSON.stringify({preflight: true, profile, profileOptions, allowAsyncFallback, basePreservation, frozenFilesVerified: frozen.size, runtimeFilesVerified: runtime.size, implementationLabels: {current: 'Current G Combined / b9142b2', candidate: `G diagnostic (same combined options)`}, phases: plans.map(plan => ({id: plan.id, images: plan.sources.length, masks: plan.masks.length, cases: plan.cases.length}))}));
  process.exit(0);
}
const out = path.join(workspace, 'experiments/efficiency-lab/qa/output', `readback-matched-${new Date().toISOString().replaceAll(':', '-')}`);
await fs.mkdir(out, {recursive: true});
const report = {schema: diagnosticCase ? 'ar-efficiency-lab-bounded-repeat-v1' : 'ar-g-readback-matched-v1', complete: false, passed: false, createdAt: new Date().toISOString(),
  base, currentModule, candidateModule, profile, profileOptions, allowAsyncFallback, basePreservation, implementationLabels: {current: 'Current G Combined / b9142b2', candidate: `G diagnostic (same combined options)`}, out, phases: [], runtimeHashes: Object.fromEntries(runtime),
  ...
  limits: ['No face or hair inference is rerun. Renderer wall times do not establish live FPS, face/hair contention, sustained mobile speed or physical-camera latency.',
    ...
    'The production live scheduler, asynchronous worker contention and profile switching are checked separately.',
    'The prior Test1 warmed current-renderer protected-pixel variance is retained and is not resolved by passing this later G/efficiency-lab still comparison.']};
const save = () => {
  for (const phase of report.phases) phase.mechanismSummary = summarizeMechanisms(phase.cases);
  report.mechanismSummary = summarizeMechanisms(report.phases.flatMap(phase => phase.cases));
  return fs.writeFile(path.join(out, 'report.json'), JSON.stringify(report, null, 2));
};
const stats = values => {
...
try {
  for (const plan of plans) {
    ...
    measured.environment = await page.evaluate(async parameters => {
      const {createMatchedStudy} = await import('/experiments/efficiency-lab/qa/readback-browser.mjs');
      ...
      window.matchedStudy = await createMatchedStudy({...parameters, renderer: environment.renderer});
      return environment;
    }, {currentModule, candidateModule, generated: plan.generated, profile, allowAsyncFallback});
    ...
    measured.inputControls = await page.evaluate(async () => {const {inputControls} = await import('/experiments/efficiency-lab/qa/input-controls.mjs'); return await inputControls();});
    assert.equal(measured.inputControls.passed, true);
    ...
    for (const eyewear of ['tom-ford-clear', 'amber-horizon']) {
      ...
      for (const input of modelCases) {
        ...
        measured.cases.push(row); await save();
      ...
      if (profileOptions.cropBranchReadback) {
        const input = modelCases.find(row => row.expectedGeometry.rearDrop.dropM > 0);
        assert.ok(input, `No nonzero branch for the ${eyewear} region diagnostic ownership control.`);
        const regionDiagnostic = await page.evaluate(input => window.matchedStudy.regionDiagnostic(input), input);
        measured.controls.push({eyewear, regionDiagnostic}); await save();
        assert.equal(regionDiagnostic.passed, true, 'Cropped branch diagnostic lost the retained source, pose, full raw pixels or visible output.');
      }
    ...
    for (const required of ['/experiments/speed-lab/native/renderer.ts', '/experiments/efficiency-lab/g-readback-diagnostic/native/renderer.ts',
      '/experiments/speed-lab/native/shared-readback.ts', '/experiments/speed-lab/speed-options.ts', '/references/perfect-temples/src/render/eyewear.ts'])
    ...
    for (const required of ['/experiments/speed-lab/native/pbo-readback.ts', '/experiments/efficiency-lab/g-readback-diagnostic/native/pbo-readback.ts'])
      assert.ok(measured.moduleUrls.some(url => new URL(url).pathname === required), `Actual PBO dependency not loaded: ${required}`);
    for (const row of measured.cases) {
      const currentPbo = row.current.stats.candidatePerformance.nativePipeline.native.speedLab.pbo;
      const candidatePbo = row.candidate.stats.candidatePerformance.nativePipeline.native.speedLab.pbo;
      assert.equal(currentPbo?.diagnosticVersion, undefined);
      assert.equal(candidatePbo?.diagnosticVersion, 1);
      assert.ok(candidatePbo.diagnosticClockReads > 0);
    }
    if (profileOptions.prewarmTemples) for (const eyewear of ['tom-ford-clear', 'amber-horizon']) {
      const rows = measured.cases.filter(row => row.eyewearModel === eyewear);
      assert.equal(rows.filter(row => row.candidate.stats.candidatePerformance.nativePipeline.speedLab.prewarmAttempted).length, 1,
        'Prewarm must run exactly once per newly created candidate renderer, with its cost retained on that first pair.');
    }
    if (profileOptions.cropBranchReadback) for (const eyewear of ['tom-ford-clear', 'amber-horizon']) for (const hairModel of ['hair-only', 'selfie-multiclass']) {
      const rows = measured.cases.filter(row => row.eyewearModel === eyewear && row.hairModel === hairModel);
      assert.ok(rows.some(row => {
        const pipeline = row.candidate.stats.candidatePerformance.nativePipeline, region = pipeline.efficiencyLab.branchRegion;
        return region.used && region.rectangle !== null && region.readBytes > 0 && region.readBytes < region.fullBytes
          && pipeline.branchReadbackCalls === 1 && pipeline.branchReadbackBytes === region.readBytes;
      }), `No positive cropped branch download was exercised for ${plan.id}/${eyewear}/${hairModel}.`);
    }
    if (profileOptions.omitBranchLenses) for (const eyewear of ['tom-ford-clear', 'amber-horizon']) {
      const rows = measured.cases.filter(row => row.eyewearModel === eyewear);
      // A vacuous final-image comparison cannot establish that a lens was
