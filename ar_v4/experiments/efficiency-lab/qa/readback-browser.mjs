/** Diagnostic G versus accepted G. Both use the exact combined options. Browser-only matched QA. Imports the two actual implementations; never detects faces or hair. */
import {createOwnedSourceFrame} from '../../speed-lab/speed-options.ts';
import {PROFILES as G_PROFILES} from '../../speed-lab/profiles.ts';
import {checkProtocol as checkG, mechanismUse, noAuxiliaryCamera} from '../../speed-lab/qa/protocol.mjs';
const checkProtocol = (_name, options, stats, geometry, width, height, policy) => checkG('candidate', options, stats, geometry, width, height, policy);
export async function createMatchedStudy({currentModule, candidateModule, generated, profile, allowAsyncFallback = false, renderer}) {
  const options = G_PROFILES.combined.options;
