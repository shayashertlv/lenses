...
export class PboNativeReadback {
  ...
  constructor(renderer: WebGLRenderer | HTMLCanvasElement) {
    ...
    if (!gl || !('fenceSync' in gl)) throw new Error('WebGL 2 pixel-pack buffers are unavailable.');
    this.gl = gl;
  }
  ...
  async finish(isCurrent: () => boolean, timeoutMs = 500): Promise<{beauty: ImageData; camera: ImageData | null}> {
    ...
    try {
      ...
      const images = this.withPackState(() => Array.from({length: this.count}, (_, index) => {
        ...
        if (this.poolScratch && this.scratch.byteLength === slot.bytes) {
          bottom = this.scratch; this.measured.scratchBytesReused += bottom.byteLength;
        } else {
          bottom = new Uint8Array(slot.bytes); this.measured.scratchBytesAllocated += bottom.byteLength;
          if (this.poolScratch) this.scratch = bottom;
        }
        ...
        this.measured.outputBytesAllocated += top.byteLength;
