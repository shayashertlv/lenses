...
export interface FastHairOptions {
  /** X only: compare all four initial RGBA bytes as one aligned word. */
  wordCompose?: boolean;
...
export function composeHairArmsFast(input: HairArmInput, options: FastHairOptions = {}): FastHairArmResult {
  ...
  const result: FastHairArmResult = {pixels: before.slice(), weights: new Float32Array(collectWeights ? width * height : 0), fallbackReason: null,
    ...
    wordComparisonRequested: options.wordCompose === true, wordComparisonUsed: false, wordComparedPixels: 0};
  ...
  try {
    ...
    const wordComparison = options.wordCompose === true && before.byteOffset % 4 === 0 && background.byteOffset % 4 === 0
