...
import {CATEGORY_EXTRACTION_PROTOCOL, assertCategoryExtractionMode, validateCategoryExtractionMetrics} from './protocol.ts';
...
export class HairClient {
  ...
  async segment(image: ImageBitmap, sourceSHA256: string, sequence: number, categoryExtractionMode: CategoryExtractionMode = 'sdk'): Promise<HairSegmentationResult> {
    try {
      ...
      const expected = {sourceSHA256, sequence, width: image.width, height: image.height, delegate: this.delegate, outputMode: this.outputMode, categoryExtractionMode};
      const result = await this.request({type: 'segment', sessionNonce: this.sessionNonce, requestId: this.nextId++, image, sourceSHA256, sequence, categoryExtractionMode},
  ...
  private async handleMessage(message: unknown): Promise<void> {
    ...
    try {
      ...
      const categoryExtraction = validateCategoryExtractionMetrics(isRecord(message.output) ? message.output.categoryExtraction : undefined,
        pending.expected, output.extractionMs);
      const result: HairSegmentationResult = output.outputMode === 'category-only'
        ? {...output, ...await hashHairMasks(output), categoryExtraction} : {...output, ...await hashHairMasks(output), categoryExtraction};
