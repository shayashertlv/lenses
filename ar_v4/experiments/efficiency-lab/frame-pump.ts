...
export interface FramePumpOptions<Frame, Inference, Prepared> {
  ...
  /** Release overlap only at this pair's explicit native-submission milestone. */
  readonly deferPrefetch?: boolean;
  ...
  prepare(frame: Frame, inference: Inference, signal: AbortSignal, beginPrefetch: () => void): Prepared | Promise<Prepared>;
...
export interface FramePumpStats {
  ...
  readonly prefetchStarts: number;
  readonly deferredSubmissions: number;
...
export class FramePump<Frame, Inference, Prepared> {
  ...
  private counts = {offered: 0, captured: 0, captureMisses: 0, published: 0, replaced: 0,
    ...
    prefetchStarts: 0, deferredSubmissions: 0};
  ...
  offer(captureNow: () => Frame | null): boolean {
    ...
    try {
      ...
      const slot: Slot<Frame, Inference, Prepared> = {frame, identity, abort: new AbortController(),
        ...
        prefetchAllowed: !this.options.deferPrefetch};
      frame = null;
      this.owned.add(slot);
  ...
  private prefetch(): void {
    if (this.options.mode === 'overlap' && this.accepting && this.active?.phase === 'preparing'
      && this.active.prefetchAllowed && this.pending?.phase === 'captured' && !this.inferring) {
      this.counts.prefetchStarts++; void this.infer(this.pending);
    }
  }
  ...
  private async process(slot: Slot<Frame, Inference, Prepared>): Promise<void> {
    try {
      ...
      slot.busy++;
      try {
        const beginPrefetch = (): void => {
          // A late callback from a cancelled or completed pair cannot open the
          // next pair's gate. The callback is deliberately idempotent.
          if (!this.live(slot) || this.active !== slot || slot.phase !== 'preparing' || slot.prefetchAllowed) return;
          slot.prefetchAllowed = true; this.counts.deferredSubmissions++; this.prefetch();
        };
        slot.prepared = await this.options.prepare(slot.frame, slot.inference as Inference, slot.abort.signal, beginPrefetch);
