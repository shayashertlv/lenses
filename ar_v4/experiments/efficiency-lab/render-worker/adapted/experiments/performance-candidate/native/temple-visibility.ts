...
export function createTempleVisibility(root: Object3D, context: VisibilityContext) {
  ...
  const frontalUniforms = {
    ...
    templeFrontalFront: {value: frontZM}, templeFrontalCameraSource: {value: null as CanvasTexture<OffscreenCanvas> | null},
  ...
  return {
    ...
    prepare(rawMatrix: readonly number[], cameraTexture?: CanvasTexture<OffscreenCanvas>): void {
      if (disposed) throw new Error('Temple visibility is disposed.');
      restoreColorWrites();
