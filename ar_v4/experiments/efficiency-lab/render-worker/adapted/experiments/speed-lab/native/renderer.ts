...
import { createTempleVisibility, createTempleVisibilityConfiguration, TEMPLE_VISIBILITY_METHOD,
  TEMPLE_VISIBILITY_PARAMETERS } from '../../performance-candidate/native/temple-visibility.ts';
import type { TempleVisibilityConfiguration } from '../../performance-candidate/native/temple-visibility.ts';
import {SharedNativeReadback} from '../../../../../../speed-lab/native/shared-readback.ts';
import type {SharedReadbackMetrics} from '../../../../../../speed-lab/native/shared-readback.ts';
import {PboNativeReadback} from '../../../../../../speed-lab/native/pbo-readback.ts';
import type {PboReadbackMetrics} from '../../../../../../speed-lab/native/pbo-readback.ts';
...
export interface NativeStageTimings {
  setupMs: number; visibilitySubmitMs: number; beautySubmitMs: number; cleanSubmitMs: number;
  readbackSetupMs: number; fallbackBeautySubmitMs: number; totalMs: number;
  requestedCamera: boolean; sharedCameraReady: boolean; sharedCameraFailure: string | null;
  sharedReadback: SharedReadbackMetrics | null;
  speedLab: {reuseSourcePixelsRequested: boolean; reuseSourcePixelsUsed: boolean; sourceReuseFallback: string | null;
    asyncReadbackRequested: boolean; asyncReadbackUsed: boolean; asyncFallback: string | null;
    sourceEligibilityMs: number; pbo: PboReadbackMetrics | null};
}
const emptyStageTimings = (): NativeStageTimings => ({setupMs: 0, visibilitySubmitMs: 0, beautySubmitMs: 0,
  cleanSubmitMs: 0, readbackSetupMs: 0, fallbackBeautySubmitMs: 0, totalMs: 0,
  requestedCamera: false, sharedCameraReady: false, sharedCameraFailure: null, sharedReadback: null,
  speedLab: {reuseSourcePixelsRequested: false, reuseSourcePixelsUsed: false, sourceReuseFallback: null,
    asyncReadbackRequested: false, asyncReadbackUsed: false, asyncFallback: null, sourceEligibilityMs: 0, pbo: null}});
...
export class TryOnRenderer {
  ...
  private readonly cleanScene = new Scene();
  private pairedReadback: SharedNativeReadback | null = null;
  private nativePair: {beauty: ImageData; camera: ImageData | null} | null = null;
  ...
  private activeOptions: NativeSpeedOptions = DEFAULT_SPEED_OPTIONS;
  private pbo: PboNativeReadback | null = null;
  private pboQueued = false;
  private awaiting = false;
  private pairGeneration = 0;
  private stages = emptyStageTimings();
  private backgroundTexture: CanvasTexture<OffscreenCanvas> | null = null;
  ...
  /** Borrowed immutable pixels of the latest complete native pair. */
  get nativeImagePair(): {beauty: ImageData; camera: ImageData | null} | null {return this.disposed ? null : this.nativePair;}
  setFrameOptions(options: NativeSpeedOptions, source?: OwnedSourceFrame): void {
    if (this.awaiting) throw new Error('A pending native pair cannot change options.');
    this.nextOptions = normalizeSpeedOptions(options); this.nextSource = source;
  }
  get stageTimings(): NativeStageTimings {return structuredClone(this.stages);}
  ...
  static async create(canvas: OffscreenCanvas, signal: AbortSignal, eyewearId = DEFAULT_EYEWEAR_ID): Promise<TryOnRenderer> {
  ...
  present(frame: OffscreenCanvas, detection: Detection, recordedSurface?: readonly number[],
    recordedTempleClip?: TempleClipConfiguration | null,
    recordedTempleVisibility?: TempleVisibilityConfiguration | null, captureCamera = false): boolean {
    if (this.awaiting) throw new Error('The preceding native pair is pending.');
    return this.presentInternal(frame, detection, recordedSurface, recordedTempleClip, recordedTempleVisibility, captureCamera, false);
  }

  async presentAsync(frame: OffscreenCanvas, detection: Detection, recordedSurface?: readonly number[],
    recordedTempleClip?: TempleClipConfiguration | null,
    recordedTempleVisibility?: TempleVisibilityConfiguration | null, captureCamera = false): Promise<boolean> {
    if (this.disposed) return false;
    if (this.awaiting) throw new Error('The preceding native pair is pending.');
    const started = performance.now();
    const visible = this.presentInternal(frame, detection, recordedSurface, recordedTempleClip, recordedTempleVisibility, captureCamera, true);
    if (!this.activeOptions.asyncReadback) return visible;
    const source = this.activeSource, options = this.activeOptions, generation = this.pairGeneration;
    this.awaiting = true;
    try {
      if (!this.pboQueued || !this.pbo) throw new Error(this.stages.speedLab.asyncFallback ?? 'The PBO path was unavailable.');
      const pair = await this.pbo.finish(() => !this.disposed && generation === this.pairGeneration && (!source || source.isCurrent()));
      if (this.disposed || generation !== this.pairGeneration || source && !source.isCurrent()) throw abortError();
      if (this.renderer.getContext().isContextLost()) throw new Error('The native context was lost before retrieval could commit.');
      this.nativePair = {beauty: pair.beauty, camera: this.stages.speedLab.reuseSourcePixelsUsed ? source!.rgba : pair.camera};
      this.stages.sharedCameraReady = this.nativePair.camera !== null;
      this.stages.speedLab.asyncReadbackUsed = true;
      this.stages.speedLab.pbo = this.pbo.metrics;
      this.stages.totalMs = performance.now() - started;
      return visible;
    } catch (error) {
      const metrics = this.pbo?.metrics ?? null;
      if (this.disposed || generation !== this.pairGeneration || source && !source.isCurrent()
        || error instanceof DOMException && error.name === 'AbortError') {
        this.clearPresentation(); throw new DOMException('The native pair was cancelled.', 'AbortError');
      }
      if (this.renderer.getContext().isContextLost()) {this.clearPresentation(); throw error;}
      const failedStages = this.stages;
      this.awaiting = false;
      this.setFrameOptions({...options, asyncReadback: false}, source);
      const fallback = this.presentInternal(frame, detection, recordedSurface, recordedTempleClip, recordedTempleVisibility, captureCamera, false);
      for (const key of ['setupMs', 'visibilitySubmitMs', 'beautySubmitMs', 'cleanSubmitMs', 'readbackSetupMs', 'fallbackBeautySubmitMs'] as const)
        this.stages[key] += failedStages[key];
      this.stages.speedLab.sourceEligibilityMs += failedStages.speedLab.sourceEligibilityMs;
      this.stages.speedLab.asyncReadbackRequested = true;
      this.stages.speedLab.asyncFallback = error instanceof Error ? error.message : String(error);
      this.stages.speedLab.pbo = metrics; this.stages.totalMs = performance.now() - started;
      return fallback;
    } finally {this.awaiting = false;}
  }

  private presentInternal(frame: OffscreenCanvas, detection: Detection, recordedSurface?: readonly number[],
    recordedTempleClip?: TempleClipConfiguration | null,
    recordedTempleVisibility?: TempleVisibilityConfiguration | null, captureCamera = false, allowAsync = false): boolean {
    if (this.disposed) return false;
    this.clearPresentation();
    this.activeOptions = this.nextOptions; this.activeSource = this.nextSource;
    this.nextOptions = DEFAULT_SPEED_OPTIONS; this.nextSource = undefined;
    if (this.activeSource && !this.activeSource.isCurrent()) throw new DOMException('The native source is stale.', 'AbortError');
    const started = performance.now(); this.stages = emptyStageTimings(); this.stages.requestedCamera = captureCamera;
    const options = this.activeOptions, source = this.activeSource;
    this.stages.speedLab.reuseSourcePixelsRequested = options.reuseSourcePixels;
    this.stages.speedLab.asyncReadbackRequested = options.asyncReadback;
    if (options.asyncReadback && !allowAsync) this.stages.speedLab.asyncFallback = 'Synchronous presentation requested.';
    ...
    try {
      ...
      this.stages.setupMs = performance.now() - started;
      ...
      this.stages.visibilitySubmitMs = performance.now() - visibilityStarted;
      const beautyStarted = performance.now();
      this.renderer.render(this.scene, this.camera);
      this.stages.beautySubmitMs = performance.now() - beautyStarted;
      const wantsCamera = captureCamera && this.facePose.visible;
      const asyncCapture = options.asyncReadback && allowAsync;
      if (wantsCamera && options.reuseSourcePixels) {
        const eligibilityStarted = performance.now();
        const identity = [1, 0, 0, 0, 1, 0, 0, 0, 1];
        const textureIdentity = this.backgroundTexture.colorSpace === SRGBColorSpace
          && this.backgroundTexture.matrix.elements.every((value, index) => value === identity[index])
          && this.scene.backgroundIntensity === 1 && this.cleanScene.backgroundIntensity === 1;
        const reason = sourceCameraEligibility(source, renderWidth, renderHeight)
          ?? (textureIdentity ? null : 'Native background transform/color settings are not the identity path.');
        this.stages.speedLab.sourceReuseFallback = reason;
        this.stages.speedLab.reuseSourcePixelsUsed = reason === null;
        this.stages.speedLab.sourceEligibilityMs = performance.now() - eligibilityStarted;
      }
      if (wantsCamera || asyncCapture) {
        try {
          const readbackStarted = performance.now();
          if (asyncCapture) {this.pbo ??= new PboNativeReadback(this.renderer); this.pbo.begin(renderWidth, renderHeight);}
          ...
          if (asyncCapture) this.pbo!.capture(0); else this.pairedReadback!.capture(0);
          ...
          if (asyncCapture) this.pboQueued = true;
          else this.nativePair = this.stages.speedLab.reuseSourcePixelsUsed
            ? {beauty: this.pairedReadback!.readBeauty(), camera: source!.rgba} : this.pairedReadback!.read();
          this.stages.sharedCameraReady = !asyncCapture && !!this.nativePair?.camera;
        } catch (error) {
          // The optional optimization never licenses stale/missing native pixels.
          // Recreate the complete original beauty for the ordinary fallback read.
          this.nativePair = null;
          this.pboQueued = false; this.pbo?.cancel();
          if (asyncCapture) this.stages.speedLab.asyncFallback = error instanceof Error ? error.message : String(error);
          this.stages.sharedCameraFailure = error instanceof Error ? error.message : 'Shared camera capture failed';
          const fallbackStarted = performance.now();
          this.renderer.render(this.scene, this.camera);
          this.stages.fallbackBeautySubmitMs = performance.now() - fallbackStarted;
        } finally {
          this.cleanScene.background = null;
          this.stages.sharedReadback = asyncCapture ? null : this.pairedReadback?.metrics ?? null;
          this.stages.speedLab.pbo = asyncCapture ? this.pbo?.metrics ?? null : null;
        }
      }
      this.stages.totalMs = performance.now() - started;
  ...
  private clearPresentation(): void {
    this.pairGeneration++; this.pbo?.cancel(); this.pboQueued = false;
    this.nativePair = null;
  ...
  dispose(): void {
    ...
    this.cleanScene.background = null;
    this.nativePair = null; this.pairedReadback?.dispose(); this.pairedReadback = null;
    this.pbo?.dispose(); this.pbo = null; this.pairGeneration++;
