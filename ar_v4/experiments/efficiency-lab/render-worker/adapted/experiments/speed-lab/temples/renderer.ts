...
interface PreparedInput {frame: OffscreenCanvas; paired: Detection; options: NativeSpeedOptions; source?: OwnedSourceFrame;
...
export class TryOnRenderer {
  ...
  private readonly baselineCanvas: OffscreenCanvas;
  private readonly branchCanvas: OffscreenCanvas;
  private readonly display: OffscreenCanvas;
  private readonly displayContext: OffscreenCanvasRenderingContext2D;
  private readonly pairedFrame = new OffscreenCanvas(300, 150);
  private readonly pairedContext = context2D(this.pairedFrame);
  private readonly baselineReadback = new OffscreenCanvas(300, 150);
  private readonly baselineContext = context2D(this.baselineReadback);
  private readonly branchReadback = new OffscreenCanvas(300, 150);
  ...
  private presentationFrame: OffscreenCanvas | null = null;
  ...
  private constructor(display: OffscreenCanvas, baselineCanvas: OffscreenCanvas, branchCanvas: OffscreenCanvas,
  ...
  static async create(display: OffscreenCanvas, signal: AbortSignal, eyewearId: EyewearId = DEFAULT_EYEWEAR_ID,
    options: RendererOptions = {}): Promise<TryOnRenderer> {
    if (signal.aborted) throw abortError();
    const baselineCanvas = new OffscreenCanvas(300, 150), branchCanvas = new OffscreenCanvas(300, 150);
  ...
  private takeInput(frame: OffscreenCanvas, detection: Detection, previousBorrowedPixels?: ImageData): PreparedInput {
  ...
  private get ownedFrame(): OffscreenCanvas {
  ...
  async exportDiagnostic(): Promise<Record<string, unknown> | null> {
    ...
    const output = new OffscreenCanvas(300, 150); output.width = this.candidatePixels.width; output.height = this.candidatePixels.height;
    context2D(output).putImageData(this.candidatePixels, 0, 0);
    const candidatePngDataUrl = await canvasPng(output);
    ...
    return {schema: 'ar_v4_sagittal_diagnostic_v1', exportedAt: new Date().toISOString(), sequence: this.sequence,
      ...
      sourcePngDataUrl: await canvasPng(this.ownedFrame), detection: structuredClone(this.pairedDetection),
      originalPerfectoPngDataUrl: await canvasPng(this.baselineReadback), candidatePngDataUrl,
      branchPngDataUrl: this.branchPixels ? await canvasPng(this.branchReadback) : null,
