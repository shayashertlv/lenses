...
export interface HeldHairInput {source: OffscreenCanvas; detection: Detection; pair: PairIdentity; expectedModel: HairModelContract;}
...
const context = (canvas: OffscreenCanvas): OffscreenCanvasRenderingContext2D => {
...
const png = async (value: OwnedPixels): Promise<string> => {
  const canvas = new OffscreenCanvas(300, 150); canvas.width = value.width; canvas.height = value.height;
  context(canvas).putImageData(new ImageData(new Uint8ClampedArray(value.pixels), value.width, value.height), 0, 0); return await canvasPng(canvas);
};
...
export class LiveHairRenderer {
  private readonly displayContext: OffscreenCanvasRenderingContext2D;
  private readonly ownedSourceCanvas = new OffscreenCanvas(300, 150);
  ...
  private get source(): OffscreenCanvas {
  ...
  private constructor(private readonly display: OffscreenCanvas, private readonly acceptedCanvas: OffscreenCanvas,
    private readonly accepted: AcceptedRenderer) {
    this.displayContext = context(display);
  }

  static async create(display: OffscreenCanvas, signal: AbortSignal, eyewearId: EyewearId = DEFAULT_EYEWEAR_ID): Promise<LiveHairRenderer> {
    if (signal.aborted) throw abortError();
    const acceptedCanvas = new OffscreenCanvas(300, 150);
  ...
  async present(frame: OffscreenCanvas, detection: Detection, mask: HairMask | null, pair: PairIdentity,
  ...
  async prepare(frame: OffscreenCanvas, detection: Detection, pair: PairIdentity, expectedModel: HairModelContract,
  ...
  copyHeldInput(): HeldHairInput | null {
    ...
    const source = new OffscreenCanvas(300, 150); source.width = this.source.width; source.height = this.source.height;
  ...
  async exportDiagnostic(): Promise<Record<string, unknown> | null> {
    ...
    return {schema: 'hair-live-diagnostic-v1', acceptedCommit: 'b26b5584c0dccbc2b30e4f12cdd432f10df577ea',
      ...
      width: before.width, height: before.height, sourcePngDataUrl: await canvasPng(this.source),
      acceptedPngDataUrl: await png(before), hairPngDataUrl: await png(after), cleanBackgroundPngDataUrl: this.background ? await png(this.background) : null,
