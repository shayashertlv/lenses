...
export function createTempleClip(root: Object3D) {
  ...
  const cameraSource = {value: null as CanvasTexture<OffscreenCanvas> | null};
  ...
  return {
    ...
    prepareRender(cameraTexture?: CanvasTexture<OffscreenCanvas>, width?: number, height?: number): void {
      if (disposed) throw new Error('Temple clipping is disposed.');
      if (fadeMode.value === 3) {
