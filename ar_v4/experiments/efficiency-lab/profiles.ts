...
export const PIPELINES = ['g', 'scratch', 'lean', 'deferred', 'temples', 'combined', 'rate12', 'rate10', 'rate8', 'mask', 'publish', 'region', 'lens', 'ui'] as const;
...
export const PIPELINE_LABELS: Record<Pipeline, string> = {
  ...
  lens: 'S · Lean temple render', ui: 'T · Lighter statistics',
};
...
const profile = (extra: {poolReadbackScratch?: boolean; asyncTemples?: boolean; cropBranchReadback?: boolean; omitBranchLenses?: boolean}, detail: string,
  ...
  throttleUi = false) => Object.freeze({options: normalizeSpeedOptions({...g, ...extra}),
  mode: 'overlap' as const, leanInputs, deferPrefetch, captureRateHz, hairExtractionMode, suppressUnchangedPublication, throttleUi, detail});
...
export function usesBaseRenderer(id: Pipeline): boolean {return id === 'g' || id === 'lean' || id === 'mask' || id === 'publish' || id === 'ui' || PROFILES[id].captureRateHz !== null;}
...
export function studyPipelines(search: string): readonly Pipeline[] {
  ...
  return study === 'review' ? ['g', 'publish', 'region', 'lens', 'ui'] : study === 'mask' ? ['g', 'mask'] : PIPELINES;
}
