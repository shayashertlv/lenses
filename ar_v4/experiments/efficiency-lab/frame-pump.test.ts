...
function harness(mode: 'fresh' | 'overlap' = 'fresh', deferPrefetch = false) {
  ...
  const submit = new Map<number, () => void>();
  ...
  const options: FramePumpOptions<Frame, Inference, Prepared> = {
    ...
    prepare: async (frame, inference, signal, beginPrefetch) => {
      ...
      submit.set(frame.sequence, beginPrefetch);
  ...
  return {pump, options, frames, inferences, prepared, inferenceGates, prepareGates, signals, submit, trace, published,
...
test('deferred overlap waits for this pair native submission, then infers the newest pair and locks it', async () => {
  const h = harness('overlap', true); h.prepareGates.set(1, deferred<void>());
  h.offer(1); h.offer(2); h.resolveInference(1); await setImmediate();
  assert.deepEqual(h.trace.filter(value => value.startsWith('infer:')), ['infer:1']);
  h.offer(3); assert.equal(h.frames[1]!.closed, 1); assert.equal(h.maxOwned(), 2);
  h.submit.get(1)!(); h.submit.get(1)!();
  assert.deepEqual(h.trace.filter(value => value.startsWith('infer:')), ['infer:1', 'infer:3']);
  assert.equal(h.pump.stats.deferredSubmissions, 1); assert.equal(h.pump.stats.prefetchStarts, 1);
  assert.equal(h.offer(4), false); assert.equal(h.pump.stats.lockedDrops, 1);
  h.resolveInference(3); await setImmediate(); h.prepareGates.get(1)!.resolve(); await h.pump.whenIdle();
  assert.deepEqual(h.published, [1, 3]); assert.equal(h.pump.stats.maxInFlightInference, 1);
  assert.equal(h.owned(), 0); assert.deepEqual(h.errors, []);
});

test('deferred submission before a later camera offer opens only the current preparation gate', async () => {
  const h = harness('overlap', true); h.prepareGates.set(1, deferred<void>()); h.prepareGates.set(2, deferred<void>());
  h.offer(1); h.resolveInference(1); await setImmediate(); h.submit.get(1)!(); h.offer(2);
  assert.deepEqual(h.trace.filter(value => value.startsWith('infer:')), ['infer:1', 'infer:2']);
  h.resolveInference(2); h.prepareGates.get(1)!.resolve(); await setImmediate(); h.offer(3);
  h.submit.get(1)!();
  assert.deepEqual(h.trace.filter(value => value.startsWith('infer:')), ['infer:1', 'infer:2']);
  h.submit.get(2)!(); h.resolveInference(3); h.prepareGates.get(2)!.resolve(); await h.pump.whenIdle();
  assert.deepEqual(h.published, [1, 2, 3]); assert.equal(h.pump.stats.deferredSubmissions, 2);
});

test('stop before deferred submission prevents late prefetch and publication without closing active resources early', async () => {
  const h = harness('overlap', true); h.prepareGates.set(1, deferred<void>());
  h.offer(1); h.offer(2); h.resolveInference(1); await setImmediate(); h.pump.stop();
  assert.equal(h.frames[0]!.closed, 0); assert.equal(h.frames[1]!.closed, 1);
  h.submit.get(1)!(); assert.equal(h.pump.stats.deferredSubmissions, 0);
  h.prepareGates.get(1)!.resolve(); await h.pump.whenIdle();
  assert.deepEqual(h.trace.filter(value => value.startsWith('infer:')), ['infer:1']);
  assert.deepEqual(h.published, []); assert.equal(h.owned(), 0);
});

test('Hold before deferred submission drains the active pair and never sends the discarded pair to a worker', async () => {
  const h = harness('overlap', true); h.prepareGates.set(1, deferred<void>());
  h.offer(1); h.offer(2); h.resolveInference(1); await setImmediate(); const held = h.pump.finishCurrent();
  h.submit.get(1)!(); h.prepareGates.get(1)!.resolve(); await held;
  assert.deepEqual(h.trace.filter(value => value.startsWith('infer:')), ['infer:1']);
  assert.deepEqual(h.published, [1]); assert.equal(h.owned(), 0);
});

test('no-submission preparation still processes its pending pair serially without deadlock', async () => {
  const h = harness('overlap', true); h.offer(1); h.offer(2); h.resolveInference(1); await setImmediate();
  assert.deepEqual(h.published, [1]);
  assert.deepEqual(h.trace.filter(value => value.startsWith('infer:')), ['infer:1', 'infer:2']);
  h.resolveInference(2); await h.pump.whenIdle(); assert.deepEqual(h.published, [1, 2]);
  assert.equal(h.pump.stats.prefetchStarts, 0); assert.equal(h.pump.stats.deferredSubmissions, 0);
});
