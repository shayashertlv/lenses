...
import {validatedWorkerTiming} from '../../performance-stage2/face-timing.ts';
import type {FaceDelegate, FaceStageTiming, TimedDetectorRequest, TimedDetectorResponse} from '../../performance-stage2/face-timing.ts';

export type { Detection, Landmark } from '../../../references/perfect-temples/src/runtime/protocol.ts';
export type {FaceStageTiming} from '../../performance-stage2/face-timing.ts';
...
/** Structural contract shared with the unchanged accepted detector. */
export interface FaceDetector {
  readonly delegate: FaceDelegate | null;
  readonly lastTiming: FaceStageTiming | null;
  initialize(signal: AbortSignal): Promise<void>;
  detect(image: ImageBitmap, timestampMs: number): Promise<Detection>;
  close(): void;
}

/** Isolated delegate candidate; failure is reported, never relabelled as a CPU run. */
export class ForcedDelegateDetectorClient implements FaceDetector {
  ...
  private readonly requestedDelegate: FaceDelegate;

  constructor(requestedDelegate: FaceDelegate, options: DetectorOptions = {}) {
    if (requestedDelegate !== 'CPU' && requestedDelegate !== 'GPU') throw new Error('Unknown requested face delegate.');
    this.requestedDelegate = requestedDelegate;
  ...
  async initialize(signal: AbortSignal): Promise<void> {
    ...
    try {
      for (const delegate of [this.requestedDelegate]) {
        if (signal.aborted) throw cancelled();
        this.createWorker();
        try {
          await this.request({
  ...
  private createWorker(): void {
    const worker = this.options.createWorker?.() ?? new Worker(new URL('../../performance-stage2/face-detector.worker.ts', import.meta.url), { type: 'module' });
    this.worker = worker;
  ...
  private handleMessage(message: TimedDetectorResponse): void {
    ...
    if (message.type === 'error') {
      this.settle(new Error(message.message || 'Face detection failed.'));
      return;
    }
