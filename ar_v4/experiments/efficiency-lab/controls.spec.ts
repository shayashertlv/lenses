...
interface CategoryBeforeHold {
  ...
  capturedAtMs: number; workerDetectionJSON: string; fullMaskRequestSequence: number;
  categoryBase64: string; acceptedPngDataUrl: string; hairPngDataUrl: string;
}
interface ControlState {
  armNoFaceHold: boolean; noFaceHold: {previousSerial: number; previousSequence: number; requested: boolean; requestedPipeline:string} | null;
  ...
  categoryHoldRequest: {sequence: number; sourceSHA256: string; detectionSHA256: string} | null;
  faceResults: {capturedAtMs: number; detectionJSON: string}[];
...
async function installCamera(page: Page): Promise<void> {
  ...
  await page.addInitScript(() => {
    ...
    const state: ControlState = {streams: [], workers: [], armPendingHold: false, armAsyncStop: false,
      armNoFaceHold: false, noFaceHold: null,
      ...
      upgradeWasLocked: false, upgradeExportWasNull: false, categoryResults: [], categoryHoldRequest: null, faceResults: []};
    ...
    WebGL2RenderingContext.prototype.clientWaitSync = function(sync, flags, timeout): GLenum {
      ...
      if (value.phase !== 'live' || value.pipeline !== 'temples' || value.processing !== true || value.stats !== null
      ...
      setTimeout(() => {
        ...
        select.value = 'g'; select.dispatchEvent(new Event('change'));
    ...
    class ObservedWorker extends NativeWorker {
      ...
      constructor(url: string | URL, options?: WorkerOptions) {
        ...
        this.addEventListener('message', (event: MessageEvent<Record<string, unknown>>) => {
          if(state.armCategoryHold&&event.data.type==='result'&&event.data.detection){
            const timing=event.data.timing as {timestampMs:number}|undefined;
            if(timing&&Number.isFinite(timing.timestampMs)){
              state.faceResults.push({capturedAtMs:timing.timestampMs,detectionJSON:JSON.stringify(event.data.detection)});
              if(state.faceResults.length>6)state.faceResults.shift();
            }
          }
          if (state.armNoFaceHold && event.data.type === 'result' && event.data.detection && window.hairLivePreview) {
            const value=window.hairLivePreview.diagnostics(), hold=document.querySelector<HTMLButtonElement>('#hold-frame')!;
            const stats=value.stats as {hasFace?:boolean}|null;
            // Gate a real active face reply before the client consumes it. This
            // intentionally injects one no-face result for the pending Hold pair.
            // Current deferred preparation has not started; a prefetched reply
            // during private rendering is excluded by the non-null stats check.
            if(value.phase==='live'&&value.pipeline==='deferred'&&value.processing===true&&stats?.hasFace&&!hold.disabled){
              state.armNoFaceHold=false;event.stopImmediatePropagation();
              const previousSequence=(value.presented as {sequence:number}).sequence;
              const previousSerial=window.arPerformanceProfiler.samplesAfter(0).at(-1)!.serial;
              hold.click();
              const select=document.querySelector<HTMLSelectElement>('#pipeline-select')!;
              select.value='g';select.dispatchEvent(new Event('change'));
              state.noFaceHold={previousSerial,previousSequence,requested:window.hairLivePreview.diagnostics().holdRequested===true,requestedPipeline:select.value};
              const response={...event.data,detection:{...(event.data.detection as Record<string,unknown>),landmarks:[],matrix:null}};
              setTimeout(()=>{if(!observed.terminated)this.onmessage?.call(this,new MessageEvent('message',{data:response}));},0);
              return;
            }
          }
          ...
          if (state.armOverlapStop && window.hairLivePreview) {
            const value = window.hairLivePreview.diagnostics();
            if (value.phase === 'live' && value.pipeline === 'lean' && value.processing === true && value.presented) {
      ...
      override postMessage(message: unknown, transfer: Transferable[] | StructuredSerializeOptions = []): void {
        ...
        if (request.type === 'segment' && state.armPrefetchToggle && window.hairLivePreview) {
          ...
          if (value.phase === 'live' && value.pipeline === 'lean' && value.processing === true && value.stats === null
        ...
        if (request.type === 'segment' && state.failHeldSegment && window.hairLivePreview.diagnostics().phase === 'held') {
          // Hold drains its active pair, which can follow the image visible at
          // the click. Observe that exact final live pair before injecting the
          // full-mask failure, independently matching both real worker results.
          const value=window.hairLivePreview.diagnostics();
          const pair=value.presented as {sequence:number;sourceSHA256:string;detectionSHA256:string;capturedAtMs:number};
          const category=state.categoryResults.find(item=>item.sequence===pair.sequence&&item.sourceSHA256===pair.sourceSHA256);
          const face=state.faceResults.find(item=>item.capturedAtMs===pair.capturedAtMs);
          const stats=value.stats as {hasMask?:boolean;maskOutputMode?:string}|null;
          if(!category||!face||!stats?.hasMask||stats.maskOutputMode!=='category-only'||request.sequence!==pair.sequence)
            throw new Error('QA could not independently identify the exact held category/detection pair before upgrade.');
          ...
          variant.value='hair';variant.dispatchEvent(new Event('change'));const hairPngDataUrl=canvas.toDataURL('image/png');
          variant.value='accepted';variant.dispatchEvent(new Event('change'));const acceptedPngDataUrl=canvas.toDataURL('image/png');
          variant.value='hair';variant.dispatchEvent(new Event('change'));
          let binary='';for(let offset=0;offset<category.category.length;offset+=0x4000)
          ...
          state.beforeFailedUpgrade={...pair,pipeline:value.pipeline as string,categoryBase64:btoa(binary),
            workerDetectionJSON:face.detectionJSON,fullMaskRequestSequence:request.sequence,acceptedPngDataUrl,hairPngDataUrl};
          state.armCategoryHold=false;state.categoryResults.length=0;state.faceResults.length=0;
          ...
          // Speed lab populates its six-output cache after the upgrade attempt;
    ...
    setInterval(() => {
      ...
      const pair = value.presented as {sequence: number; sourceSHA256: string; detectionSHA256: string; capturedAtMs:number};
      const category = state.categoryResults.find(item => item.sequence === pair.sequence && item.sourceSHA256 === pair.sourceSHA256);
      if (!category||!state.faceResults.some(item=>item.capturedAtMs===pair.capturedAtMs)||state.categoryHoldRequest) return;
      state.categoryHoldRequest={sequence:pair.sequence,sourceSHA256:pair.sourceSHA256,detectionSHA256:pair.detectionSHA256};
      state.failHeldSegment = true; held.click();
    }, 1);
  });
}
...
test('pending async download preserves displayed ownership through switch request and Hold; all six held outputs toggle exactly', async ({page}) => {
  const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
  await installCamera(page); await page.goto('/experiments/efficiency-lab/live.html');
  await page.selectOption('#eyewear-select', 'tom-ford-clear'); await page.selectOption('#pipeline-select', 'temples');
  await page.click('#start'); await waitCategory(page, 'temples');
  ...
  expect(observation.preparedPipeline).toBe('temples'); expect(observation.requestedPipeline).toBe('g');
  ...
  await expect(page.locator('.stage')).toHaveAttribute('data-pipeline', 'temples');
  await expect(page.locator('#pipeline-select')).toHaveValue('temples');
  ...
  expect(comparison.schema).toBe('ar-efficiency-comparison-v1'); expect(comparison.selectedPipeline).toBe('temples');
  expect(comparison.candidateAccepted).toBe(false);
  for (const pipeline of PIPELINES) {
    expect(comparison[pipeline].pair).toEqual(comparison.g.pair);
    expect(comparison[pipeline].detection).toEqual(comparison.g.detection);
    expect(comparison[pipeline].sourcePngDataUrl).toBe(comparison.g.sourcePngDataUrl);
    expect(comparison[pipeline].mask).toEqual(comparison.g.mask);
  ...
  await writeFile(test.info().outputPath('pending-controls-receipt.json'), JSON.stringify({observation, pair: comparison.g.pair,
    exactHairToggles: [...PIPELINES], liveCategoryObserved: ['temples'], heldFullUpgradeWhileAccepted: true, errors,
...
test('no-face result during Hold then switch resumes one live pump and permits a later exact Hold', async ({page}) => {
  const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));
  await installCamera(page);await page.goto('/experiments/efficiency-lab/live.html');
  await page.selectOption('#pipeline-select','deferred');await page.click('#start');await waitCategory(page,'deferred');
  const sessionId=await page.locator('.stage').getAttribute('data-session-id');
  await page.evaluate(()=>{window.speedLabControls.armNoFaceHold=true;});
  await expect.poll(()=>page.evaluate(()=>window.speedLabControls.noFaceHold)).not.toBeNull();
  const observed=await page.evaluate(()=>window.speedLabControls.noFaceHold!);expect(observed.requested).toBe(true);
  expect(observed.requestedPipeline).toBe('g');
  await expect.poll(()=>page.evaluate(serial=>{
    const rows=window.arPerformanceProfiler.samplesAfter(serial),lost=rows.find(row=>!row.hasFace);
    return !!lost&&rows.some(row=>row.serial>lost.serial&&row.hasFace&&row.hasMask);
  },observed.previousSerial)).toBe(true);
  expect(await page.locator('.stage').getAttribute('data-session-id')).toBe(sessionId);
  await waitCategory(page,'g');expect(await page.evaluate(()=>window.hairLivePreview.diagnostics().hairError)).toBeNull();
  expect(await page.evaluate(()=>window.hairLivePreview.diagnostics().holdRequested)).toBe(false);
  await expect(page.locator('#hold-frame')).toBeEnabled();await page.click('#hold-frame');await completeHold(page);
  const output=await page.evaluate(()=>window.hairLivePreview.exportDiagnostic()) as HeldComparison;
  for(const pipeline of PIPELINES){expect(output[pipeline].pair).toEqual(output.g.pair);expect(output[pipeline].mask).toEqual(output.g.mask);}
  await inspectHairToggles(page,output);expect(errors).toEqual([]);
  await writeFile(test.info().outputPath('no-face-hold-receipt.json'),JSON.stringify({observed,sessionId,errors,
    recoveredWithoutRestart:true,subsequentExactHold:true,scope:'One deliberate no-face response at a real pending face-result boundary; real workers/rendering otherwise. Lifecycle test, not visual-motion evidence.'},null,2));
  await page.click('#stop');await expect.poll(()=>resourcesClosed(page)).toBe(true);
});

for (const pipeline of ['temples', 'lean'] as const) test(`stop during ${pipeline} work revokes the old session and allows a fresh masked restart`, async ({page}) => {
  const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
  await installCamera(page); await page.goto('/experiments/efficiency-lab/live.html');
  ...
  if (pipeline === 'lean') {
    ...
    await expect.poll(() => page.evaluate(sequence => window.arPerformanceProfiler.samplesAfter(0)
      .some(row => row.pipeline === 'lean' && row.sequence === sequence), prefetchedSequence)).toBe(true);
    const packet = await page.evaluate(sequence => window.arPerformanceProfiler.samplesAfter(0)
      .find(row => row.pipeline === 'lean' && row.sequence === sequence)!, prefetchedSequence);
  ...
  await page.evaluate(mode => {
    window.speedLabControls.armAsyncStop = mode === 'temples'; window.speedLabControls.armOverlapStop = mode === 'lean';
  }, pipeline);
  ...
  expect(observation.trigger).toBe(pipeline === 'temples' ? 'native-fence' : 'worker-result');
  if (pipeline === 'temples') expect(observation.privateStatsNull).toBe(true);
...
test('reversed combined-versus-base measurement completes automatically and exports ordered summaries with factual coverage', async ({page}) => {
  ...
  await expect(page.locator('#benchmark-results tr').nth(1)).toHaveAttribute('data-pipeline', 'g');
  await expect(page.locator('#pipeline-select')).toHaveValue('g');
  await expect(page.locator('#benchmark')).toBeDisabled();
  ...
  expect(exported.schema).toBe('ar-efficiency-performance-v1'); expect(exported.benchmark.completed).toBe(true);
  expect(exported.benchmark.cancelledReason).toBeNull(); expect(exported.benchmark.order).toEqual(['combined', 'g']);
...
test('failed full held-mask upgrade preserves the actual live category pixels and unlocks all six cached comparisons and download', async ({page}) => {
  const errors: string[] = []; page.on('pageerror', error => errors.push(error.message));
  await installCamera(page); await page.goto('/experiments/efficiency-lab/live.html');
  await page.selectOption('#pipeline-select', 'scratch'); await page.click('#start'); await waitCategory(page, 'scratch');
  ...
  const result = await page.evaluate(() => ({before: window.speedLabControls.beforeFailedUpgrade!,
    holdRequest:window.speedLabControls.categoryHoldRequest!,
  ...
  expect(comparison.schema).toBe('ar-efficiency-comparison-v1'); expect(comparison.selectedPipeline).toBe('scratch');
  const categorySHA256 = createHash('sha256').update(Buffer.from(before.categoryBase64, 'base64')).digest('hex');
  const detectionSHA256=createHash('sha256').update(before.workerDetectionJSON).digest('hex');
  expect(before.detectionSHA256).toBe(detectionSHA256);
  expect(before.sequence).toBeGreaterThanOrEqual(result.holdRequest.sequence);
  expect(before.fullMaskRequestSequence).toBe(before.sequence);
  for (const pipeline of PIPELINES) {
    ...
    expect(output.detection).toEqual(JSON.parse(before.workerDetectionJSON));
    expect(output.mask.sourceSHA256).toBe(before.sourceSHA256);expect(output.mask.detectionSHA256).toBe(detectionSHA256);
    ...
    expect(output.mask).toEqual(comparison.g.mask); expect(output.sourcePngDataUrl).toBe(comparison.g.sourcePngDataUrl);
  ...
  expect(saved.schema).toBe('ar-efficiency-comparison-v1');
  ...
  await writeFile(test.info().outputPath('failed-upgrade-controls-receipt.json'), JSON.stringify({pair: comparison.g.pair,
    holdRequest:result.holdRequest,heldSequence:before.sequence,fullMaskRequestSequence:before.fullMaskRequestSequence,detectionSHA256,
    categorySHA256, retainedLivePixelsAndMask: true, liveCategoryObserved: ['scratch'], exactHairToggles: [...PIPELINES],
