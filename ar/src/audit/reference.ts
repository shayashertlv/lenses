/** CPU binary-category diagnostic reference and four protection checks for held-frame audits.
 *  Live rendering never reads pixels back or runs this reference. GPU edge feathering differs. */
...
export function composeHairArms(input: HairArmInput, options: ComposeOptions = {}): HairArmResult {
  ...
  const result: HairArmResult = {pixels: before.slice(), weights: new Float32Array(collectWeights ? width * height : 0), fallbackReason: null,
    backgroundReferenceCheck: emptyCheck(), statistics: emptyStatistics(), regions: null};
