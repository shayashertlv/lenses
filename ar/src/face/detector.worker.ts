...
/** Face landmarker worker: MediaPipe FaceLandmarker (VIDEO mode, one face, facial transformation matrix) on the
 *  requested delegate. Replies carry the session nonce and a timing sidechannel. */
import {FaceLandmarker, FilesetResolver} from '@mediapipe/tasks-vision';
import {errorMessage, validateDetection} from './protocol.ts';
import type {DetectorResponse} from './protocol.ts';
...
scope.onmessage = async (event: MessageEvent<TimedDetectorRequest>) => {
  ...
  try {
    ...
    if (message.type === 'initialize') {
      ...
      const detection = validateDetection({landmarks, matrix, inferenceMs});
