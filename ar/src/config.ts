...
import {DEFAULT_HAIR_START_Z_M} from './render/hair-occlusion.ts';
...
export interface Config {
   ...
   *  `?steadydepthbeta=` added Hz per cm/s (0..5). Defaults follow movement responsively while keeping the 1 Hz resting cutoffs. */
...
export const DEFAULT_CONFIG: Readonly<Config> = Object.freeze({
  ...
  guard: true, eyewear: null, hairModel: null, hair: null, diagnostics: false,
...
export function describeConfig(config: Config, userAgent = typeof navigator === 'undefined' ? '' : navigator.userAgent,
  ...
  return [
    ...
    'fixed temples with hair-covered endings',
