/** The hair worker boundary: request/response envelopes and the validation of a category mask. The segmenter runs in
 *  IMAGE mode and returns the category mask only (the pipeline never uses confidence); every output is copied and
 *  checked here before the pipeline sees it. */
import {getHairModel} from './models.ts';
import type {HairModel, HairModelId} from './models.ts';

export type HairDelegate = 'CPU' | 'GPU';

/** The minimum a consumer needs: category winners at the mask's own size, hair = `hairIndex`. */
export interface CategoryMask {
  width: number; height: number; hairIndex: number; category: Uint8Array;
}
export interface HairOutput extends CategoryMask {
  sourceSHA256: string;
  sequence: number;
  model: HairModelId;
  modelSHA256: string;
...
export type HairSegmentationResult = HairOutput & {categorySHA256: string};
/** What the renderer receives: the segmentation paired with the same frame's detection identity. */
export type HairMask = HairSegmentationResult & {detectionSHA256: string};

export interface HairExpectedPair {sourceSHA256: string; sequence: number; width: number; height: number; delegate: HairDelegate;}
interface Envelope {sessionNonce: string; requestId: number;}
export type HairWorkerRequest = Envelope & (
  | {type: 'initialize'; modelId: HairModelId; delegate: HairDelegate}
  | {type: 'segment'; image: ImageBitmap; sourceSHA256: string; sequence: number}
);
export type HairWorkerResponse = Envelope & (
  | {type: 'ready'; model: HairModelId; modelSHA256: string; labels: string[]; hairIndex: number; runningMode: 'IMAGE'; delegate: HairDelegate; initializationMs: number}
  | {type: 'result'; output: HairOutput}
...
export function assertDimensions(width: unknown, height: unknown): void {
  ...
  'The hair image dimensions are invalid or exceed the bounded size.');
}
...
export function validateHairReady(value: unknown, model: HairModel, delegate: HairDelegate): void {
...
export function validateHairOutput(value: unknown, model: HairModel, expected: HairExpectedPair): HairOutput {
  ...
  check(value.delegate === expected.delegate, 'The hair output came from another inference delegate.');
  ...
  return {sourceSHA256: value.sourceSHA256, sequence: value.sequence, model: model.id, modelSHA256: model.sha256,
...
export function validateHairRequest(value: unknown): HairWorkerRequest {
  ...
  if (value.type === 'initialize') {
    ...
    check(value.delegate === 'CPU' || value.delegate === 'GPU', 'Unknown hair inference delegate.');
    return {type: 'initialize', sessionNonce: value.sessionNonce, requestId: value.requestId, modelId: model.id, delegate: value.delegate};
  }
...
export async function hashHairMask(output: HairOutput): Promise<{categorySHA256: string}> {
  return {categorySHA256: await sha256(output.category.slice())};
}
