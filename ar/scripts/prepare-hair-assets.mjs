/** Verifies the two hair segmentation weights in public/models/hair offline: each file's length and SHA-256 against the
 *  reviewed pins below, manifest.json against the same pins (and the path the page serves it from), and the license copy.
 *  With --download it first restores a missing weight from its pinned Google URL, checking length and hash before writing;
 *  it never overwrites a file that is present.
 *    node scripts/prepare-hair-assets.mjs [--download] */
...
if (manifest.schema !== 'ar-hair-model-assets-v1' || manifest.models.length !== pins.length)
...
for (const pin of pins) {
  ...
  // servingPath is relative to the page's base URL (src/assets.ts): /models/hair/… locally, /ar/models/hair/… published.
  if (!model || model.path !== filename || model.servingPath !== `models/hair/${filename}` || model.bytes !== pin.bytes
      || model.sha256 !== pin.sha256 || model.sourceUrl !== pin.sourceUrl || model.license !== 'Apache-2.0' || model.modified !== false)
...
console.log(JSON.stringify({schema: 'ar-hair-assets-verification-v1', verified: true, models: results}, null, 2));
