...
import {HairClient} from '../src/hair/client.ts';
import type {HairWorkerPort} from '../src/hair/client.ts';
import {HAIR_MODELS} from '../src/hair/models.ts';
import type {HairModelId} from '../src/hair/models.ts';
import type {HairDelegate, HairWorkerRequest} from '../src/hair/protocol.ts';
...
class FakeWorker implements HairWorkerPort {
  ...
  result(patch: Record<string, unknown> = {}, envelope: Record<string, unknown> = {}): void {
    ...
    this.emit({type: 'result', sessionNonce: message.sessionNonce, requestId: message.requestId, output: {
      ...
      labels: [...model.labels], hairIndex: 1, width: 2, height: 2, category: new Uint8Array([0, 1, 1, 0]), delegate: 'CPU',
...
async function ready(model: HairModelId = 'hair-only', delegate: HairDelegate = 'CPU') {
  const worker = new FakeWorker(), controller = new AbortController();
  const client = new HairClient(model, {createWorker: () => worker, delegate});
  const startup = client.initialize(controller.signal); worker.ready({delegate}); await startup;
  return {worker, controller, client};
}
const source = 'a'.repeat(64);

test('initialization cancellation terminates the worker and ignores a late ready reply', async () => {
...
test('the initialize request carries the delegate, and the ready reply must acknowledge it', async () => {
  const worker = new FakeWorker(), client = new HairClient('hair-only', {createWorker: () => worker});
  ...
  if (initialize.type === 'initialize') assert.equal(initialize.delegate, 'CPU');
  const rejected = assert.rejects(startup, /pinned IMAGE\/CPU/); worker.ready({delegate: 'GPU'}); await rejected;
});
...
test('the category bytes are owned copies', async () => {
  ...
  assert.deepEqual([...result.category], [0, 1, 1, 0]); assert.equal(image.closed(), 1); client.close();
});
...
test('close during asynchronous mask hashing rejects rather than publishing a stale result', async () => {
...
test('an explicitly selected GPU must acknowledge and produce GPU output', async () => {
  const {worker, client} = await ready('hair-only', 'GPU');
...
test('fabricated confidence data is refused: the pipeline is category-only', async () => {
  for (const patch of [{confidence: new Float32Array(4)}, {confidenceSHA256: 'b'.repeat(64)}]) {
    const {worker, client} = await ready(), image = bitmap();
    const pending = client.segment(image.image, source, 0);
    worker.result(patch); const result = await pending;
