...
test('pose steadiness is on by default with its levers; ?steady=0 turns it off', () => {
  ...
  assert.match(describeConfig(parseConfig('?steady=0&diag=1')), /pose steadiness OFF \(\?steady=0\) · temple occlusion per pixel[^·]+· temple fit original \(\?fit=width for the experiment\) · diagnostics ON \(\?diag=1\)\.$/);
});
