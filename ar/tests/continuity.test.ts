...
import {buildTempleContinuityModel, continuityCut, projectTempleContinuity} from '../src/render/continuity.ts';
import type {ProjectedTemplePath, TempleContinuityModel} from '../src/render/continuity.ts';
import {findDetachedTemplePixels} from '../src/audit/reference.ts';
import type {ContinuityInput} from '../src/audit/reference.ts';
...
test('reference: a hair-created middle gap removes only the downstream tip on an originally connected arm', () => {
...
test('reference: opposite yaw ordering removes the leftward distal piece instead of assuming right means terminal', () => {
...
test('reference: an existing fade/depth disconnection does not become a new continuity cut', () => {
...
test('reference: connected partial-alpha hair and unchanged/blank hair controls never terminate an arm', () => {
...
test('reference: near/far overlap is rejected and sparse callers cannot include nose, optics, outside corridor or clean camera pixels', () => {
...
test('reference: missing/invalid ordering and invalid dimensions fail closed without mutating the hair output', () => {
...
function armModel(): {root: Group; model: TempleContinuityModel; dispose(): void} {
  ...
  return {root, model: buildTempleContinuityModel(root, -.1), dispose: () => {for (const resource of [left, right, lens, frame, lensMaterial]) resource.dispose();}};
}
...
test('the cut starts at the first consistent hair run along the arm, ignores shorter strands and leaves clean arms uncut', () => {
  const {model, dispose} = armModel();
  // A yawed pose so one arm projects long across the image; the mask is half the render size.
  const pose = new Matrix4().makeRotationY(35 * Math.PI / 180).setPosition(0, 0, -40).toArray();
  const render = {width: 1200, height: 800};
  const paths = projectTempleContinuity(model, {eyewearMatrix: pose, offsetCm: [0, 3.271027, 6.531958919387042], sourceAspect: 1.5, ...render, dropM: 0})!;
  const mask = {width: 600, height: 400, hairIndex: 1, category: new Uint8Array(600 * 400)};
  assert.deepEqual(continuityCut(model, paths, mask, render, 10), {negative: null, positive: null}, 'a clean mask cuts nothing');
  const arm = [...paths].sort((a, b) => b.lengthPx - a.lengthPx)[0]!, other = paths.find(path => path !== arm)!;
  const stations = model.sides[arm.side]!, side: 'negative' | 'positive' = stations[0]!.centerXM < 0 ? 'negative' : 'positive';
  const otherSide = side === 'negative' ? 'positive' : 'negative';
  const hingeFirst = [...stations.keys()].sort((a, b) => stations[b]!.zM - stations[a]!.zM);
  const hinge = arm.points[hingeFirst[0]!]!, tip = arm.points[hingeFirst.at(-1)!]!;
  assert.ok(Math.abs(tip.x - hinge.x) > 60, 'the arm projects long enough to walk');
  // Hair is a tube of discs along the arm's centreline on the tip side of a boundary halfway along it (mask coordinates).
  const boundary = (hinge.x + tip.x) / 4, tipward = Math.sign(tip.x - hinge.x);
  const paintDiscs = (indices: readonly number[], radius: number): void => {
    mask.category.fill(0);
    for (const index of indices) {
      const point = arm.points[index]!, cx = Math.round(point.x / 2), cy = Math.round(point.y / 2);
      for (let dy = -radius; dy <= radius; dy++) for (let dx = -radius; dx <= radius; dx++) {
        if (dx * dx + dy * dy > radius * radius) continue;
        const x = cx + dx, y = cy + dy; if (x >= 0 && x < mask.width && y >= 0 && y < mask.height) mask.category[y * mask.width + x] = 1;
      }
    }
  };
  const otherTouched = (): boolean => other.points.some(point => mask.category[Math.round(point.y / 2) * mask.width + Math.round(point.x / 2)] === 1);
  const painted = hingeFirst.filter(index => (arm.points[index]!.x / 2 - boundary) * tipward > 0);
  assert.ok(painted.length >= 8 && painted.length <= stations.length - 8, 'the boundary splits the arm');
  paintDiscs(painted, 3);
  const cut = continuityCut(model, paths, mask, render, 10);
  if (!otherTouched()) assert.equal(cut[otherSide], null, 'the other arm is untouched');
  assert.ok(cut[side] !== null, 'the covered arm is cut');
  const cutOrder = hingeFirst.indexOf(stations.findIndex(station => station.zM === cut[side]));
  const firstPainted = hingeFirst.indexOf(painted[0]!);
  assert.ok(cutOrder >= firstPainted - 1 && cutOrder <= firstPainted, `the cut is the first painted station or its neighbour (cut order ${cutOrder}, first painted ${firstPainted})`);
  assert.ok(cut[side]! < stations[hingeFirst[0]!]!.zM && cut[side]! > stations[hingeFirst.at(-1)!]!.zM, 'the cut is strictly inside the arm');
  // One disc of hair on the arm (a strand a couple of stations long) is shorter than the run: no cut at 10 px, a cut at 1 px.
  paintDiscs([hingeFirst[20]!], 3);
  assert.ok(!otherTouched(), 'the strand does not touch the other arm');
  assert.deepEqual(continuityCut(model, paths, mask, render, 10), {negative: null, positive: null});
  assert.ok(continuityCut(model, paths, mask, render, 1)[side] !== null);
  dispose();
});
