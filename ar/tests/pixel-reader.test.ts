...
import {PixelReader} from '../src/render/pixel-reader.ts';

test('the reader preserves top-down bytes without a 2D conversion and owns prior frames across reuse and resize', t => {
  ...
  const canvas = {get width() {return width;}, get height() {return height;},
    getContext: (kind: string) => {
      assert.equal(kind, 'webgl2', 'reading never requests a canvas-converted image');
      return {RGBA: 1, UNSIGNED_BYTE: 2, NO_ERROR: 0, getError: () => 0, isContextLost: () => lost,
