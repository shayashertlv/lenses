...
def generate_product_description(product: dict) -> str:
    ...
    frame_desc = (
        f"{frame['thickness']} {frame['finish']} {frame_colors} {frame['material']} "
        f"{frame['rim_type']} frame with a {frame['shape']} shape"
    )
    ...
    lens_desc = (
        f"{lenses['size']} {lenses['shape']} {lens_colors} {lens_types} lenses"
    )
