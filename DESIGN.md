...
### Dispensing Docket
The wait surface: a 900px panel in three parts. A `--field` header carries the docket number and the job kind. The body is the customer on the left — a 192px 3:4 photo well, cropped to fill so the panel does not change shape with the upload's aspect, beside a key/value rule stack of the face readings stretched to the same height. The chosen frame is pulled out into a card-stock slot on the right at a fixed 260px, with its own brand line, spec row and a signal price block pinned to its foot. A hairline foot runs the full width and holds the pace — label, tabular percent, `scaleX` bar — and the cancel. Both columns carry skeleton placeholders from the first frame, since the read lands at about second eight and the match just after; each is replaced in place, so the docket never changes height and never stands part empty. Every reveal is opacity plus a small translate; nothing slides in from off-screen.

### Result Card
The shelf-edge language applied to a rendered try-on. Same paper card on a hairline over a card-stock photo well fixed at 4:5 — the well is the constant and the render is centred in it, never cropped, so three renders that come back at three shapes still give three identical cards. A `--field` label chip pins to the top-left of the well, marking the picture rather than costing the body a line. The body takes the 17px condensed brand voice, then the model name, then a spec row that reserves its second line whether or not it uses it; score chips sit on card stock with an `--ink-3` key beside a tabular value, and the foot pairs the signal price block with the product thumbnail. The body grows but never shrinks, so slack aligns the price rows across the row and a squeeze goes to the well instead of clipping text. The primary result takes an ink border and no extra size — it is meant to be compared, not enlarged. Stacked one per row below 560 the well caps at 58vh, and at 70vh on a landscape phone, so the next card still peeks. While a try-on is pending the well shows an uppercase wait line over a 72px indeterminate bar whose 45% segment translates across — again transform, never width.

**The Height-First Results Rule.** On the Smart fit results screen the row of three is bounded by height, not width: a 4:5 well over a 186px body cannot fill a 1320px row on a laptop, and pinning the row to any single width leaves either letterboxed pictures or a band of unused paper under them. The container width is derived instead — `clamp(880px, 240vh - 806px, 1320px)` — so the screen fills exactly at every viewport height, and the profile strip shares that measure so the page has one left and right edge. Below 900 this reverts to an ordinary scrolling document, where width is the binding constraint again.
