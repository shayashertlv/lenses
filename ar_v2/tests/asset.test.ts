...
/** The built asset for a catalogue id, or a failure the caller can read. */
function buildAsset(id: string): FrameAsset {
  const entry = catalogueEntry(id)!;
  const built = frameFromMesh(load(entry.file), entry);
  assert.ok(built.ok, `${id} refused: ${built.ok ? '' : built.reason}`);
  return built.asset;
}
